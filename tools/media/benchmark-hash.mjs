...
if (args.length && (args.length !== 2 || args[0] !== '--baseline'))
  throw Error('Usage: benchmark-hash.mjs [--baseline path]');
const implementations = { current: Sha256 };
if (args.length) implementations.baseline = (await import(pathToFileURL(resolve(args[1])))).Sha256;
...
for (let i = 0; i < blocks; i++) reference.update(block);
...
const execute = (Impl) => {
  ...
  if (digest !== expected) throw Error('Incorrect SHA-256 output');
  return milliseconds;
};
...
for (let n = 0; n < iterations; n++)
  for (const [name, Impl] of n % 2
    ? Object.entries(implementations).reverse()
    : Object.entries(implementations))
    timings[name].push(execute(Impl));
const report = {
  scope: 'Node incremental SHA-256 only, not MOSS or browser performance',
    ...
    Object.entries(timings).map(([name, ms]) => {
      ...
      return [
        name,
        {
          ...
          mebibytesPerSecond: bytes / 2 ** 20 / (median / 1000)
        }
      ];
    })
  )
};
if (report.results.baseline)
  report.speedup =
    report.results.baseline.medianMilliseconds / report.results.current.medianMilliseconds;
