 ...
 * All rights reserved.
 */
import { resolvePageDirection, type DirectionEvidence, type FlowSample } from '$lib/library/direction';
...
const normalize = (css: string) => css.replace(/(^|[;{])(\s*)-(?:epub|webkit)-writing-mode\s*:/gi, '$1$2writing-mode:');
const values: Record<string, RegExp> = {
  'writing-mode': /^(horizontal-tb|vertical-rl|vertical-lr|sideways-rl|sideways-lr|inherit|initial|unset|revert)$/,
  direction: /^(ltr|rtl|inherit|initial|unset|revert)$/,
  display: /^(none|contents|block|inline|inline-block|flex|inline-flex|grid|inline-grid|flow-root|list-item|table|inline-table|table-row|table-cell|table-row-group|table-header-group|table-footer-group|table-column|table-column-group|table-caption|inherit|initial|unset|revert)$/,
  visibility: /^(visible|hidden|collapse|inherit|initial|unset|revert)$/
};
function declarations(style: CSSStyleDeclaration) {
  const result: string[] = [];
  for (let index = 0; index < style.length; index++) {
    const property = style.item(index), value = style.getPropertyValue(property).trim().toLowerCase();
    if (values[property]?.test(value)) result.push(`${property}:${value}${style.getPropertyPriority(property) ? ' !important' : ''}`);
...
function stylesheet(css: string, document: Document) {
  ...
  let count = 0;
  const rules = (items: CSSRuleList): string => Array.from(items).map((rule) => {
    if (++count > 8192) throw new Error('Excessive flow rules');
    if (rule.type === 1) {
      const ordinary = rule as CSSStyleRule;
      if (ordinary.selectorText.length > 2048 || /[\\<>@{}]/.test(ordinary.selectorText)) throw new Error('Unresolved selector');
...
export function epubDirection(contents: EpubContent | EpubOPFContent, resources: Record<string, string | Blob>, ownerDocument: Document): DirectionEvidence {
  ...
  const manifest = opf ? contents['opf:package']['opf:manifest']['opf:item'] : contents.package.manifest.item;
  const refs = opf ? contents['opf:package']['opf:spine']['opf:itemref'] : contents.package.spine.itemref;
  const byId = new Map(manifest.map(item => [item['@_id'], item]));
  const documents = (Array.isArray(refs) ? refs : [refs]).filter(ref => ref && ref['@_linear'] !== 'no')
    .map(ref => byId.get(ref['@_idref'])).filter(item => item && !item['@_properties']?.split(/\s+/).includes('nav')).slice(0, 12);
  ...
  try {
    ...
    for (const item of documents) {
      ...
      const parsed = new DOMParser().parseFromString(String(resources[item['@_href']]), 'application/xhtml+xml');
      ...
      const attributes = (target: Element, original: Element) => {
        ...
        for (const name of ['id','class','dir']) if (original.hasAttribute(name)) target.setAttribute(name, original.getAttribute(name)!);
        if (original.hasAttribute('style')) target.setAttribute('style', inline(original.getAttribute('style')!, ownerDocument));
        if (original.hasAttribute('hidden')) target.setAttribute('hidden',''); else target.removeAttribute('hidden');
      };
      ...
      const clone = (source: Node, parent: Node) => {
        ...
        if (source.nodeType === Node.TEXT_NODE) {
          ...
          if (['script','style','link','meta','iframe','object','svg','math','rt','rp','nav','img','video','audio','template'].includes(tag)) return;
          const safe = /^(p|div|span|section|article|main|aside|h[1-6]|ruby|rb|ul|ol|li|table|tbody|tr|td|a|em|strong|b|i|br)$/.test(tag) ? tag : 'div';
      ...
      while ((node = walker.nextNode())) {
        ...
        samples.push({ writingMode: computed.writingMode, direction: computed.direction, characters: count });
      }
    }
