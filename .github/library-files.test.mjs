...
import { executeMove, planMove, renameSeriesOnDisk, safePath } from '../../apps/web/src/lib/library/file-operations.ts';
...
function filesystem() {
  ...
  class Directory {
    kind = 'directory';
    constructor(name, path = '') { this.name = name; this.path = path; this.children = new Map(); }
    async getDirectoryHandle(name, options = {}) {
      let value = this.children.get(name);
      if (!value && options.create) { value = new Directory(name, [this.path, name].filter(Boolean).join('/')); this.children.set(name, value); }
    ...
    async getFileHandle(name, options = {}) {
      let value = this.children.get(name);
      if (!value && options.create) { value = new Handle(name, [this.path, name].filter(Boolean).join('/')); this.children.set(name, value); }
    ...
    async removeEntry(name) {
      ...
      events.push(['delete', [this.path, name].filter(Boolean).join('/')]); this.children.delete(name);
    }
  }
  class Handle {
    kind = 'file';
    constructor(name, path) { this.name = name; this.path = path; this.bytes = new Uint8Array(); }
    ...
    async createWritable() {
      ...
      return {
        ...
        close: async () => { this.bytes = bytes; events.push(['write', this.path]); },
  ...
  return { root, events, put, read, fail: value => { failWrite = value; } };
}
async function fixture() {
  const fs = filesystem(); await fs.put('One/1.epub', 'one'); await fs.put('Two/2.epub', 'two');
...
test('physical grouping verifies all copies before relinking and removing originals', async () => {
  const fs = await fixture(), journals = [], linked = [];
  await executeMove(fs.root, fs.plan, async plan => {
  ...
  }, async file => { linked.push(file.to); });
  assert.deepEqual(journals.map(p => p.phase), ['copied', 'done']);
...
test('a failed copy keeps every original and a verified copy can be resumed', async () => {
  const fs = await fixture(); fs.fail(path => path.endsWith('2.epub'));
  await assert.rejects(executeMove(fs.root, fs.plan, async () => {}, async () => {}), /Injected write failure/);
  assert.equal(await fs.read('One/1.epub'), 'one'); assert.equal(await fs.read('Two/2.epub'), 'two');
  assert.equal(fs.events.some(([op]) => op === 'delete'), false);
  fs.fail(undefined);
  // An incomplete destination is deliberately NOT overwritten during recovery.
  await assert.rejects(executeMove(fs.root, fs.plan, async () => {}, async () => {}), /changed or is incomplete/);
  const destination = await fs.root.getDirectoryHandle('My Series');
  await destination.removeEntry('2.epub');
  await executeMove(fs.root, fs.plan, async () => {}, async () => {});
  assert.equal(await fs.read('My Series/2.epub'), 'two');
});

test('interrupted link publication resumes from durable copied journal without losing bytes', async () => {
  const fs = await fixture(); let persisted;
  await assert.rejects(executeMove(fs.root, fs.plan, async plan => { persisted = structuredClone(plan); }, async file => {
    if (file.to.endsWith('2.epub')) throw new Error('Injected link persistence failure');
  }), /link persistence/);
  assert.equal(persisted.phase, 'copied');
  await assert.rejects(fs.read('One/1.epub'), { name: 'NotFoundError' });
  assert.equal(await fs.read('My Series/1.epub'), 'one'); assert.equal(await fs.read('Two/2.epub'), 'two');
  const linked = [];
  await executeMove(fs.root, persisted, async plan => { persisted = structuredClone(plan); }, async file => { linked.push(file.to); });
  assert.equal(persisted.phase, 'done'); assert.equal(linked.length, 2);
  assert.equal(await fs.read('My Series/2.epub'), 'two');
});

test('a destination changed during asynchronous relinking stops deletion', async () => {
  const fs = await fixture();
  await assert.rejects(executeMove(fs.root, fs.plan, async () => {}, async file => {
...
test('an unrelated destination and filename collisions never overwrite files', async () => {
  const fs = await fixture(); await fs.put('My Series/other.txt', 'keep');
  await assert.rejects(executeMove(fs.root, fs.plan, async () => {}, async () => {}), /destination changed/);
  assert.equal(await fs.read('My Series/other.txt'), 'keep');
  await fs.put('Other/1.EPUB', 'different book');
  await assert.rejects(planMove(fs.root, 'source', '', 'New', ['One/1.epub', 'Other/1.EPUB']), /same filename/);
});

test('rename changes only safe metadata; root and unknown YAML are protected', async () => {
  const fs = await fixture(); await fs.put('Series/one.epub', 'book');
  await renameSeriesOnDisk(fs.root, 'Series', 'A: "name"');
  assert.equal(await fs.read('Series/.Manabi-Reader.yaml'), `name: ${JSON.stringify('A: "name"')}\n`);
...
test('tampered recovery paths are rejected before touching disk', async () => {
  const fs = await fixture(); fs.plan.files[0].to = '../outside.epub';
  await assert.rejects(executeMove(fs.root, fs.plan, async () => {}, async () => {}), /Invalid move recovery path/);
  assert.equal(fs.events.length, 0);
  for (const path of ['../a', '/a', 'a//b', 'a/../b', '.manabi-reader/a', 'a\\b']) assert.throws(() => safePath(path));
});
...
test('bounded sidecar reads enforce actual stream size as well as declared size', async () => {
  ...
  await assert.rejects(boundedBytes(new Response(bytes, { headers: { 'Content-Length': '5000' } }), 4096));
  let cancelled = false;
  const body = new ReadableStream({ start(controller) { controller.enqueue(new Uint8Array(4097)); }, cancel() { cancelled = true; } });
...
test('direction backup evidence rejects malformed, unknown and contradictory fields', () => {
  for (const value of [{value:'ltr',source:'spine'},{value:'rtl',source:'content'},{value:'unknown',source:'unknown'}])
    assert.equal(validDirectionEvidence(value), true);
  for (const value of [null, [], {}, {value:'rtl',source:'unknown'}, {value:'unknown',source:'spine'},
      {value:'rtl',source:'spine',code:'not a field'}, {value:'arbitrary',source:'content'}])
