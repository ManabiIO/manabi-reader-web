...
export function selectTtuFile<T extends { name: string }>(files: T[], prefix: string): T | undefined {
  const matches = files.filter((file) => file.name.startsWith(prefix));
  if (matches.length > 1) {
    throw new Error(`Conflicting ${prefix} files. Keep both copies for recovery; resolve the conflict before syncing.`);
  }
  ...
  if (!file.name.endsWith(extension) || parts[1] !== '1' || parts[2] !== '6') {
    throw new Error(`Unsupported TTU file format: ${file.name}. This integration supports exporter 1 / database 6.`);
  }
  ...
  const numeric = prefix === 'progress_' ? parts.slice(3, 4) : parts.slice(3, prefix === 'bookdata_' ? 6 : 16);
...
export async function resolveTtuRoot(
  ...
  try {
    ...
    if (error instanceof DOMException && error.name === 'NotFoundError') {
      throw new Error('Select the existing ttu-reader-data folder or its parent. Use Create shared library only for a new library.');
    }
    throw error;
  }
}
...
export async function inspectTtuRoot(root: FileSystemDirectoryHandle): Promise<string[]> {
  ...
  for await (const [name, handle] of root.entries()) {
    if (++entries > 10000) throw new Error('The shared library contains too many entries to inspect safely.');
    if (handle.kind !== 'directory') {
      if (ttuPrefixes.some((prefix) => name.startsWith(prefix))) {
        throw new Error('This is a book folder, not the library root. Select its parent ttu-reader-data folder.');
      }
      continue;
    }
