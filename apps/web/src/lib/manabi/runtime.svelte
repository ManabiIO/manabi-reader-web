...
{#if needsAttention && $page.url.pathname !== `${base}/connections`}
  <aside role="status"><a href="{base}/connections">Reading sync needs attention. Your local reading data is safe.</a></aside>
{/if}

<style>
  aside { position: fixed; bottom: 3.5rem; right: 1rem; z-index: 30; max-width: 22rem; padding: .65rem 1rem; border-radius: .5rem; background: #fff; color: #222; border: 1px solid #777; box-shadow: 0 2px 8px #0003; font: 14px/1.4 system-ui, sans-serif; writing-mode: horizontal-tb; }
