...
const safeProperties = new Set([
  ...
  'font-variant', 'font-variant-east-asian', 'font-kerning', 'font-feature-settings',
  'line-height', 'letter-spacing', 'word-spacing', 'text-align', 'text-indent',
  'text-decoration', 'text-decoration-line', 'text-decoration-style', 'text-decoration-color',
  'text-transform', 'text-emphasis', 'text-emphasis-style', 'text-emphasis-position',
  'text-orientation', 'text-combine-upright', 'writing-mode', 'vertical-align',
  ...
  'display', 'float', 'clear', 'box-sizing', 'list-style-type', 'list-style-position',
  'break-before', 'break-after', 'break-inside', 'page-break-before', 'page-break-after',
  'page-break-inside', 'ruby-align', 'ruby-position', 'opacity'
]);
const forbiddenTags = ['script', 'style', 'link', 'meta', 'base', 'iframe', 'frame', 'frameset',
  'object', 'embed', 'form', 'input', 'button', 'select', 'textarea', 'audio', 'video', 'source',
  'track', 'portal', 'foreignobject', 'animate', 'animatemotion', 'animatetransform', 'set', 'discard'];
...
function cleanDeclaration(style: CSSStyleDeclaration) {
  ...
  for (const property of Array.from(style)) {
    ...
    if (!safeProperties.has(normalized) || /[\\<>@]|url\s*\(|image-set\s*\(|expression\s*\(|var\s*\(/i.test(value)) continue;
...
function allowedImage(value: string) {
  if (/^data:image\/(?:png|jpeg|gif|webp|avif);(?:[A-Za-z0-9_.:/;-]+;)?base64,[A-Za-z0-9+/=\s]+$/i.test(value)) return true;
...
export function sanitizeBookHtml(html: string, importing = false, wholeDocument = false): string {
  ...
  return purifier.sanitize(html, {
    ...
    FORBID_ATTR: ['srcdoc', 'srcset', 'ping', 'target', 'download', 'formaction', 'action', 'is', 'autofocus'],
