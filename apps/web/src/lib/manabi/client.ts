...
export interface ManabiUser { id: string; username: string }
...
export class IntegrationError extends Error {
  constructor(public readonly code: string, public readonly status = 0, public readonly retryAfter = 0) {
    super(messages[code] ?? 'The connection could not complete. Your local reading data is unchanged.');
...
function invalidateAccount() {
  generation += 1;
  account.update((state) => ({ ...state, session: state.session ? { ...state.session, user: null } : null }));
}
...
async function jsonResponse(response: Response): Promise<any> {
  ...
  if (value.length > 4 * 1024 * 1024) throw new IntegrationError('invalid_response', response.status);
  try { return JSON.parse(value); } catch { throw new IntegrationError('invalid_response', response.status); }
}
...
export async function refreshAccount(): Promise<ManabiSession | null> {
  ...
  try {
    ...
    if (response.status === 404 || response.status === 503) {
      if (serial === refreshSerial) account.update((state) => ({ ...state, status: 'unavailable' }));
      return null;
    }
    ...
    if (typeof session.csrf_token !== 'string' || !Array.isArray(session.providers) ||
        !session.providers.every((p) => typeof p === 'string' && /^[a-z][a-z0-9-]{0,31}$/.test(p)) ||
        (session.user !== null && (typeof session.user?.id !== 'string' || typeof session.user?.username !== 'string'))) {
    ...
    if (serial === refreshSerial) account.update((state) => ({ ...state, status: navigator.onLine ? 'unavailable' : 'offline' }));
    return null;
  }
}

export async function request<T>(path: string, options: {
  method?: 'GET' | 'POST' | 'PUT' | 'DELETE'; value?: unknown; revision?: string;
  userId?: string; binary?: boolean;
} = {}): Promise<T> {
  if (!/^[A-Za-z0-9][A-Za-z0-9_/?=&.%:-]*$/.test(path) || path.includes('..') || path.startsWith('//')) {
    throw new Error('Invalid internal API path');
  }
  const scope = accountScope();
  if (options.userId && options.userId !== scope.userId) throw new IntegrationError('account_changed', 409);
  ...
  try {
    response = await fetch(ROOT + path, {
      method, headers, body: options.value === undefined ? undefined : JSON.stringify(options.value),
      credentials: 'same-origin', redirect: 'error', cache: 'no-store', signal: AbortSignal.timeout(45000)
    });
  } catch {
  ...
  if (responseUser !== null && responseUser !== scope.userId) {
    invalidateAccount();
  ...
  if (!response.ok) {
    ...
    throw new IntegrationError(typeof body.error === 'string' ? body.error : 'unavailable', response.status,
...
export async function connectProvider(provider: string) {
  ...
  const result = await request<{ authorize_url: string }>(`oauth/${provider}/connect/`, { method: 'POST', value: {} });
  const url = new URL(result.authorize_url);
  const httpsProvider = url.protocol === 'https:' &&
    ['accounts.google.com', 'login.microsoftonline.com', 'www.dropbox.com'].includes(url.hostname);
  const isolatedLocal = ['127.0.0.1', 'localhost'].includes(location.hostname) &&
    url.protocol === 'http:' && url.hostname === '127.0.0.1';
  if ((!httpsProvider && !isolatedLocal) || url.username || url.password) throw new IntegrationError('invalid_response');
...
export const providerLabels: Record<string, string> = {
  google: 'Google Drive', onedrive: 'OneDrive', dropbox: 'Dropbox'
};
