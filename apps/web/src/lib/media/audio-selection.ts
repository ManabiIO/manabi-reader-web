...
export function audioLanguage(value: unknown): string {
    try { return language(value); } catch { return 'und'; }
}
...
export function chooseTranscriptionAudio(tracks: readonly AudioChoice[], target: string): AudioChoice | undefined {
    const wanted = language(target), available = tracks.filter(t => t.decodable);
    const exact = available.filter(t => audioLanguage(t.language) === wanted);
    if (exact.length) return automatic(exact);
    const family = available.filter(t => audioLanguage(t.language).split('-')[0] === wanted.split('-')[0]);
