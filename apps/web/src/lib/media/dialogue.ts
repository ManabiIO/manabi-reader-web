...
export function dialogueText(cues: readonly Pick<Cue, 'text' | 'speaker'>[]): string {
    const dual = cues.every(cue => !!cue.speaker) &&
        new Set(cues.map(cue => cue.speaker)).size > 1;
...
export interface SubtitleEvent { start: number; end: number; text: string }
...
export function subtitleEvents(cues: readonly Cue[]): SubtitleEvent[] {
    ...
    if (!normalized.some(cue => cue.speaker))
        return normalized.map(cue => ({ start: cue.start / 1000, end: cue.end / 1000, text: cue.text }));

    const boundaries = normalized.flatMap((cue, index) => [
        { at: cue.start, index, start: true }, { at: cue.end, index, start: false }
    ]).sort((a, b) => a.at - b.at || Number(a.start) - Number(b.start) || a.index - b.index);
    const active = new Set<number>(), result: SubtitleEvent[] = [];
    let textBudget = 0;
    for (let i = 0; i < boundaries.length;) {
        const start = boundaries[i].at;
        do {
            const event = boundaries[i++];
            if (event.start) active.add(event.index); else active.delete(event.index);
        ...
        for (const index of active) {
            ...
            if (length > LIMITS.cueText) throw new Error('Overlapping dialogue is too large to export safely');
        }
        const turns = [...active].sort((a, b) => normalized[a].start - normalized[b].start || a - b)
