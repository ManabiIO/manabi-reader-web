...
export function assertRuntimeIdentity(runtime: RuntimeIdentity, expectedThreaded?: boolean): void {
  const fail = () => {
    throw new Error(
      'The MOSS runtime is outdated or incompatible. Rebuild and deploy both CPU runtimes before generating transcripts.'
    );
  };
  try {
    ...
    if (!Number.isSafeInteger(engine) || engine! <= 0 || !Number.isSafeInteger(ggml) || ggml! <= 0)
      fail();
    if (
      runtime.UTF8ToString(engine!) !== MOSS.engineRevision ||
      runtime.UTF8ToString(ggml!) !== MOSS.ggmlRevision
    )
      fail();
...
export function assertRuntimeBindings(value: unknown, threaded: boolean): void {
  const fail = () => {
    throw new Error(
      'The MOSS runtime is missing required bindings. Rebuild and deploy both CPU runtimes before downloading its model.'
    );
  };
  try {
    ...
    if (
      !fs ||
      !['mkdir', 'mount', 'unmount'].every((name) => typeof fs[name] === 'function') ||
      !runtime.WORKERFS
    )
      fail();
    if (
      !(runtime.HEAP32 instanceof Int32Array) ||
      !(runtime.HEAPF32 instanceof Float32Array) ||
      runtime.HEAP32.buffer !== runtime.HEAPF32.buffer
    )
      fail();
    const heap = runtime.HEAP32 as Int32Array;
    const offset = (runtime._moss_web_cancel_ptr as () => number)();
    if (!Number.isSafeInteger(offset) || offset <= 0 || offset % 4 || offset + 4 > heap.byteLength)
      fail();
    if (
      threaded &&
      typeof (runtime.PThread as { terminateAllThreads?: unknown } | undefined)
