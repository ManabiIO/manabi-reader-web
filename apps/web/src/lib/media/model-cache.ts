...
function abortTarget(target: Pick<DownloadTarget, 'abort'>) {
  ...
  catch { /* The write/verification error remains authoritative. */ }
}
export async function downloadVerified(response: Response, target: DownloadTarget, expected: {
  ...
  try {
    ...
    catch { /* Cleanup must not hide the failure. */ }
  }
}
export async function getModel(signal: AbortSignal, progress: (p: ModelProgress) => void): Promise<File> {
  ...
  const work = async () => {
    ...
    const dir = await abortable(signal, () => root.getDirectoryHandle('manabi-models', { create: true }));
    ...
    if (cached?.size === MOSS.bytes && await abortable(signal, () => hashBlob(cached!, signal, loaded => progress({ stage: 'verifying', loaded, total: MOSS.bytes }))) === MOSS.sha256)
      return cached;
    signal.throwIfAborted();
    ...
    const writer = await abortable(signal, () => handle.createWritable().then(opened => {
      ...
      if (signal.aborted) { abortTarget(opened); signal.throwIfAborted(); }
      return opened;
    }));
