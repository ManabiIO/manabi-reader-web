...
export interface StudySpan { start: number; end: number; cues: readonly Cue[] }
...
export function studySpans(cues: readonly Cue[], delay = 0): StudySpan[] {
    ...
    for (const cue of [...cues].sort((a, b) => a.start - b.start || a.end - b.end)) {
        const start = cue.start + delay, end = cue.end + delay;
...
export function spanIndex(spans: readonly StudySpan[], time: number): number {
    ...
    while (lo < hi) {
        const mid = (lo + hi) >>> 1;
        if (spans[mid].start <= time) lo = mid + 1; else hi = mid;
    }
    return lo - 1;
}

export function seekSpan(spans: readonly StudySpan[], time: number, direction: -1 | 0 | 1): StudySpan | undefined {
...
export class LinePause {
    ...
    held?: StudySpan;
    reset() { this.previous = undefined; this.target = undefined; this.held = undefined; }
    sample(time: number, playing: boolean, spans: readonly StudySpan[]): StudySpan | undefined {
        ...
        this.held = undefined;
        if (this.previous !== undefined && time >= this.previous && this.target &&
            this.previous < this.target.end && time >= this.target.end) {
