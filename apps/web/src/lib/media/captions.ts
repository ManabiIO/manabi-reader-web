...
export function parseSubtitles(source: string): Cue[] {
    ...
    const blocks = normalized.split(/\n(?:[ \t]*\n)+/), vtt = /^WEBVTT(?:[ \t].*)?(?:\n|$)/.test(normalized), out: Cue[] = [];
    if (vtt && blocks[0].includes('-->'))
        throw new Error('WebVTT header needs a blank line');
    for (const [index, block] of blocks.entries()) {
        if (vtt && (index === 0 || /^(?:NOTE(?:[ \t\n]|$)|STYLE\n|REGION\n)/.test(block)))
            continue;
        const lines = block.split('\n'), i = lines[0].includes('-->') ? 0 : 1, m = /^\s*(\S+)\s+-->\s+(\S+)(?:[ \t]+.*)?$/.exec(lines[i] ?? '');
        if (!m)
            throw new Error(`Invalid subtitle block ${index + 1}`);
        out.push(validateCue({ id: `cue-${out.length}`, start: timestamp(m[1]), end: timestamp(m[2]), text: plainCaption(lines.slice(i + 1).join('\n')) }));
        if (out.length > LIMITS.cues)
...
export class CueTimeline {
    ...
    private ends: number[];
    constructor(cues: readonly Cue[]) { this.cues = [...cues].sort((a, b) => a.start - b.start); let end = 0; this.ends = this.cues.map(c => end = Math.max(end, c.end)); }
    active(time: number, delay = 0): Cue[] {
        if (!Number.isFinite(time) || !Number.isFinite(delay))
            return [];
        time -= delay;
        ...
        for (let i = lo - 1; i >= 0 && this.ends[i] > time; i--)
            if (this.cues[i].end > time)
                result.push(this.cues[i]);
...
export function matchSidecar(video: string, name: string): {
    language: string;
    forced: boolean;
} | null {
    if (!/\.(srt|vtt)$/i.test(name))
        return null;
    const base = video.replace(/\.[^.]+$/, ''), stem = name.replace(/\.[^.]+$/, '');
    if (stem.toLowerCase() !== base.toLowerCase() && !stem.toLowerCase().startsWith(base.toLowerCase() + '.'))
        return null;
    const parts = stem.slice(base.length).split('.').filter(Boolean);
    let lang = 'und';
    for (const part of parts) {
        if (['forced', 'sdh', 'cc'].includes(part.toLowerCase()))
            continue;
        try {
            lang = language(part);
            break;
        }
        catch { /* A descriptive suffix is not a language. */ }
    }
...
export function chooseLayout(width: number, height: number, ratio: number, previous: 'side' | 'below' = 'below'): 'side' | 'below' {
    if (![width, height, ratio].every(n => Number.isFinite(n) && n > 0) || width < 700)
        return 'below';
    const pane = Math.min(420, Math.max(280, width * .3)), bottom = Math.min(300, Math.max(160, height * .3));
    const area = (w: number, h: number) => Math.min(w, h * ratio) ** 2 / ratio;
    const side = area(Math.max(1, width - pane - 16), height), below = area(width, Math.max(1, height - bottom - 16));
    return previous === 'side' ? (below > side * 1.08 ? 'below' : 'side') : (side > below * 1.08 ? 'side' : 'below');
}
