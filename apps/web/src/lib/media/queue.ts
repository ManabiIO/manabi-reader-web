...
export interface QueueProgress { job: Job; stage: string; loaded: number; total: number }
...
export class TranscriptionQueue {
    ...
    constructor(private store: MediaStore, private scope: Scope, private engine: Engine,
        private decode: (job: Job, start: number, end: number, signal: AbortSignal) => Promise<Float32Array>,
        private changed: (p: QueueProgress) => void = () => {}, private failed: (error: unknown) => void = () => {}) {
        this.stopStore = store.subscribe?.(() => { void this.checkCancellation(); });
    }
    private report(error: unknown) { if (!this.closed) { try { this.failed(error); } catch { /* UI is not durable state. */ } } }
    private notify(p: QueueProgress) { if (!this.closed) { try { this.changed(p); } catch (e) { this.report(e); } } }
    ...
    async resume(id: string) {
        ...
        await this.store.updateLocal<Job>(this.scope, 'jobs', id, old => {
            ...
            this.compatible(job);
            if (this.closed) throw new Error('The queue is closed');
            const next = releasedJob(job, 'queued'); delete next.error;
            return next;
        });
        this.kick();
    }
    async cancel(id: string) {
        if (this.active?.id === id) this.active.controller.abort(new DOMException('Generation cancelled', 'AbortError'));
        await this.store.updateLocal<Job>(this.scope, 'jobs', id, old => {
            if (!old) return old;
            const job = validateJob(old);
            if (job.id !== id) throw new Error('Wrong saved job identity');
        ...
        const recover = async (exclusive: boolean) => {
            for (const job of await this.jobs()) {
                ...
                await this.store.updateLocal<Job>(this.scope, 'jobs', job.id, old => {
                    ...
                    if (latest.status === 'queued' || (latest.status === 'running' && (exclusive || (latest.leaseUntil ?? 0) <= Date.now())))
                        return releasedJob(latest, 'paused');
                    return old;
                });
            }
        };
        if (navigator.locks) await navigator.locks.request('manabi-moss-inference', { ifAvailable: true }, async lock => { if (lock) await recover(true); });
    ...
    private checkCancellation(): Promise<void> {
        ...
        const check = (async () => {
            ...
            } catch (e) { if (this.active === active) active.controller.abort(e); }
        })();
        this.checking = check;
        void check.finally(() => {
            if (this.checking === check) this.checking = undefined;
            if (this.checkAgain) { this.checkAgain = false; void this.checkCancellation(); }
        });
        return check;
    }
    private kick() {
        ...
        try {
            for (;;) {
                ...
                const work = async () => {
                    ...
                    let job = claimed, heartbeatBusy = false;
                    const heartbeat = setInterval(() => {
                        if (heartbeatBusy || signal.aborted) return;
                        heartbeatBusy = true;
                        void this.store.updateLocal<Job>(this.scope, 'jobs', job.id, old => {
                            if (!ownsJob(old, ownerId)) throw new JobOwnershipLost();
                            return { ...old!, leaseUntil: Date.now() + JOB_LEASE_MS };
                        }).catch(e => controller.abort(e)).finally(() => { heartbeatBusy = false; });
                    }, 15_000);
                    ...
                    try {
                        ...
                        if (job.nextWindow < windows.length) await this.engine.prepare(signal, p => this.notify({ job, ...p }));
                        for (let i = job.nextWindow; i < windows.length; i++) {
                            ...
                            const raw = pcm.every(x => x === 0) ? '' : await this.engine.transcribe(pcm, signal);
                            signal.throwIfAborted();
                        ...
                        const completed = releasedJob(job, 'complete'); delete completed.error;
                        signal.throwIfAborted();
                        ...
                        if (latest && latest.status !== 'complete') this.notify({ job: latest, stage: latest.status, loaded: latest.nextWindow, total: planWindows(latest.duration).length });
                    } finally { clearInterval(heartbeat); }
                };
                try {
                    if (navigator.locks) await navigator.locks.request('manabi-moss-inference', { signal }, work);
                    else await work();
    ...
    async dispose() {
        this.closed = true; this.stopStore?.();
