...
export class MossClient {
    ...
    private retire() {
        ...
        if (worker) {
            ...
            try { worker.postMessage({ type: 'dispose' }); } catch { /* Already crashed/closed. */ }
            setTimeout(() => worker.terminate(), 50);
        }
    }
    private call(type: 'prepare' | 'transcribe', fields: Record<string, unknown>, signal: AbortSignal,
        progress: (p: ModelProgress) => void = () => {}, transfer: Transferable[] = []): Promise<unknown> {
        ...
        return new Promise((yes, no) => {
                ...
                if (event.data.type === 'progress') {
                    try {
                        ...
                        if (!p || !['checking', 'downloading', 'verifying', 'loading'].includes(p.stage) ||
                            ...
                            p.total <= 0 || p.loaded > p.total) throw new Error('Invalid model download progress');
                        progress({ stage: p.stage, loaded: p.loaded, total: p.total });
                    } catch (e) { fail(e); }
                    return;
                }
                if (event.data.type === 'error') { fail(new Error(String(event.data.value))); return; }
                try {
            ...
            const cancel = () => {
                try {
                    ...
                    else if (type !== 'prepare') { fail(signal.reason); return; }
                    timer = setTimeout(() => fail(signal.reason), type === 'prepare' ? 2000 : 15000);
                } catch { fail(signal.reason); }
            };
            worker.addEventListener('message', message);
            signal.addEventListener('abort', cancel, { once: true });
            try { signal.throwIfAborted(); worker.postMessage({ id, type, operation, ...fields }, transfer); }
            catch (e) { fail(e); }
        });
    }
