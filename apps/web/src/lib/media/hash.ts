...
export class Sha256 {
    ...
    update(bytes: Uint8Array): this {
        if (this.finished)
            throw new Error('Digest is finalized');
    ...
    hex(): string {
        if (this.finished)
            throw new Error('Digest is finalized');
        const bits = this.size * 8, pad = new Uint8Array(this.used < 56 ? 64 - this.used : 128 - this.used);
        pad[0] = 128;
...
export const digestText = (text: string) => new Sha256().update(new TextEncoder().encode(text)).hex();
export async function hashBlob(blob: Blob, signal: AbortSignal, progress: (n: number) => void = () => { }): Promise<string> {
