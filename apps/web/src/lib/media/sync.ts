...
export async function syncMedia(store: MediaStore, transport: SyncTransport, signal: AbortSignal, publish: (s: SyncStatus) => void = () => { }) {
    const scope: Scope = `account:${transport.userId}`;
    const guard = () => { signal.throwIfAborted(); if (!transport.isCurrent())
    ...
    const work = async () => {
        ...
        for (let pages = 0;; pages++) {
            guard();
            if (pages > 10000)
                throw new Error('Sync feed is unexpectedly large');
            ...
            guard();
            if (!feed || !Array.isArray(feed.items) || feed.items.length > 3 || !Number.isSafeInteger(feed.next_cursor) || feed.next_cursor < cursor || typeof feed.has_more !== 'boolean' || (feed.has_more && feed.next_cursor === cursor))
                throw new Error('Invalid sync feed');
            let previous = cursor;
            for (const item of feed.items) {
                if (!item || typeof item !== 'object' || Array.isArray(item)) throw new Error('Invalid sync record');
                const value = item as Record<string, unknown>;
                if (!Number.isSafeInteger(value.sequence) || (value.sequence as number) <= previous || (value.sequence as number) > feed.next_cursor)
        ...
        const pending = (await store.records(scope)).filter(r => r.dirty || r.pending).sort((a, b) => (a.kind === 'video_track' ? 1 : 0) - (b.kind === 'video_track' ? 1 : 0));
        for (const candidate of pending) {
            ...
            try {
                const response = await transport.request<{
                    ...
                    mutation_id: string;
                    record: unknown;
                }>('personal/mutations/', { method: 'POST', value: r.pending.request, userId: transport.userId });
                guard();
                ...
                if (error.status === 412 && error.current) {
                    await store.conflict(scope, r.kind, r.id, remote(error.current), r.pending.request.mutation_id);
                }
                else
                    throw e;
            }
        ...
        guard();
        publish({ state: conflicts.length ? 'conflict' : stillPending ? 'pending' : 'synced',
            message: conflicts.length ? 'Both copies changed. Choose which version to keep.' :
                stillPending ? 'New changes are saved on this device and waiting to sync.' : 'Video progress and subtitles synced.',
            conflicts });
    };
    try {
        if (navigator.locks)
        ...
        if (!signal.aborted && transport.isCurrent())
            publish({ state: 'error', message: error instanceof Error ? error.message : String(error), conflicts: [] });
        throw error;
    }
}
