...
export const sessionKey = (bookId: number, title: string): string => JSON.stringify([bookId, title])
export function emptySession(): AudiobookSession {
  return { version: 1, subtitleName: '', subtitleSource: '', position: 0, rate: 1, delay: 0, follow: false, approximate: false, updatedAt: 0 }
}
...
export function validateSession(value: unknown): AudiobookSession {
  ...
  if (v.version !== 1 || typeof v.subtitleName !== 'string' || v.subtitleName.length > 1024 || typeof v.subtitleSource !== 'string' || v.subtitleSource.length > MAX_SUBTITLE_BYTES || new TextEncoder().encode(v.subtitleSource).length > MAX_SUBTITLE_BYTES || !bounded(v.position, 0, 3_600_000_000) || !bounded(v.rate, 0.5, 3) || !bounded(v.delay, -3600, 3600) || !bounded(v.updatedAt, 0, Number.MAX_SAFE_INTEGER) || typeof v.follow !== 'boolean' || typeof v.approximate !== 'boolean') {
  ...
  if (v.audio !== undefined) {
    ...
    if (typeof a.name !== 'string' || a.name.length > 1024 || !bounded(a.size, 0, Number.MAX_SAFE_INTEGER) || !Number.isInteger(a.size) || !bounded(a.lastModified, 0, Number.MAX_SAFE_INTEGER)) throw new Error('Invalid saved audio identity')
    audio = { name: a.name, size: a.size, lastModified: a.lastModified }
  }
  return { version: 1, subtitleName: v.subtitleName, subtitleSource: v.subtitleSource, audio, position: v.position, rate: v.rate, delay: v.delay, updatedAt: v.updatedAt, follow: v.follow, approximate: v.approximate }
}


/** Initial metadata loading and closing a player must not erase its checkpoint. */
export function captureSession(
  draft: Pick<AudiobookSession, 'subtitleName' | 'subtitleSource' | 'delay' | 'follow' | 'approximate'>,
...
export class AudiobookSessionStore {
  ...
  private open(): Promise<IDBDatabase> {
    ...
    try { factory = typeof this.factory === 'function' ? this.factory() : this.factory ?? globalThis.indexedDB }
    catch { return Promise.reject(new Error('Browser storage is unavailable: access denied')) }
    ...
    const opening = new Promise<IDBDatabase>((resolve, reject) => {
      ...
      const timer = setTimeout(() => fail(new Error('Audiobook storage did not respond. Playback remains available.')), this.openTimeoutMs)
      ...
      request.onsuccess = () => {
        ...
        const forget = () => { if (this.database === opening) this.database = undefined }
        db.onversionchange = () => { db.close(); forget() }
        db.onclose = forget
        resolve(db)
      }
      request.onerror = () => fail(request.error ?? new Error('Could not open audiobook storage'))
      request.onblocked = () => fail(new Error('Audiobook storage is blocked by another tab. Close it and reopen this panel.'))
    })
  ...
  save(key: string, session: AudiobookSession): Promise<void> {
    ...
    try { snapshot = validateSession(session) } catch (error) { return Promise.reject(error) }
    return this.serialize(async (db) => { await this.transaction(db, 'readwrite', (store) => store.put(snapshot, key)) })
  }
  remove(key: string): Promise<void> {
    return this.serialize(async (db) => { await this.transaction(db, 'readwrite', (store) => store.delete(key)) })
  }

  private transaction(db: IDBDatabase, mode: IDBTransactionMode, action: (store: IDBObjectStore) => IDBRequest): Promise<unknown> {
    return new Promise((resolve, reject) => {
      const transaction = db.transaction('sessions', mode)
      let request: IDBRequest | undefined
      transaction.oncomplete = () => resolve(request?.result)
      transaction.onabort = () => reject(transaction.error ?? new Error('Audiobook storage transaction failed (storage may be full)'))
      try { request = action(transaction.objectStore('sessions')) }
      catch (error) {
  ...
  async close(): Promise<void> {
    this.closing = true
    await this.queue
    try { (await this.database)?.close() } catch { /* Opening failure was reported to the caller. */ }
