...
export function cueText(value: string): string {
  ...
  const text = value
    ...
    .replace(/<\/?(?:b|i|u|ruby|rt|rp|c|v|lang)(?:[.\s][^<>]*)?>/gi, '')
...
export function parseSubtitles(source: string): Cue[] {
  if (
    source.length > MAX_SUBTITLE_BYTES ||
    new TextEncoder().encode(source).length > MAX_SUBTITLE_BYTES
  ) {
  ...
  if (vtt && blocks[0].includes('-->'))
    throw new Error('Invalid WebVTT header: expected a blank-line separator before cues');
  const cues: Cue[] = [];
  for (let blockIndex = 0; blockIndex < blocks.length; blockIndex += 1) {
    ...
    cues.push({
      ...
      text: cueText(lines.slice(timingLine + 1).join('\n'))
    });
...
/** Play only the part of a caption that exists in the selected audio file. */
export function cueAudioBounds(
  cue: Cue,
  delay: number,
  duration: number
): { start: number; end: number } | undefined {
  if (![cue.start, cue.end, delay, duration].every(Number.isFinite) || duration <= 0)
    return undefined;
  const start = Math.max(0, cue.start + delay);
  const end = Math.min(duration, cue.end + delay);
