<script lang="ts">
  ...
  async function restore() {
    try {
      ...
      if (data) {
        ...
        try {
          timeline = new CueTimeline(
            data.subtitleSource ? parseSubtitles(data.subtitleSource) : []
          );
          cues = timeline.cues;
        } catch {
          error =
            'Saved subtitles could not be parsed. Select a valid subtitle file; the saved audio resume position has been retained.';
        }
        delay = data.delay;
        follow = data.follow;
      ...
      if (alive)
        storageError =
          'Saved audiobook data could not be loaded. Playback still works; use “Remove saved audiobook data” to reset this book if needed.';
    } finally {
      if (alive) ready = true;
    }
  }
  ...
  async function save() {
    ...
    try {
      ...
      if (alive && generation === saveGeneration && !clearing) {
        dirty = true;
        storageError =
          'Audiobook changes could not be saved locally. Keep a copy of your subtitles; playback remains available.';
      }
    }
  }
  ...
  function selectAudio(event: Event) {
    ...
    player.load(
      file,
      saved.audio ? { identity: audioIdentity(saved.audio), position: saved.position } : undefined
    );
  }
  ...
  async function matchBook() {
    ...
    const selected =
      selection?.rangeCount && !selection.isCollapsed ? selection.getRangeAt(0) : undefined;
    const activeRoot = document.querySelector<HTMLElement>('.book-content');
    const hint = (
      selected && activeRoot?.contains(selected.startContainer) ? selected : selectionHint
    )?.cloneRange();
    ...
    try {
      ...
      if (!root)
        throw new Error('Book content is not ready. Try matching again after it finishes loading.');
      ...
      const start =
        hint && root.contains(hint.startContainer) ? offsetForSelection(built, hint) : undefined;
      ...
      if (
        ...
        matchController !== controller ||
        root !== document.querySelector('.book-content') ||
        !built.isCurrent()
      )
        return;
      index = built;
  ...
  function contentChanged(html: string, layout: string | number) {
    ...
    invalidateMatches(
      hasMatched || matching
        ? 'The reader layout changed. Select “Match book” to rebuild highlighting for this layout.'
        : ''
    );
  }
  ...
  function navigateToRange(range: Range) {
    ...
    if (bookmark) {
      onFollow();
      bookmarkManager.scrollToBookmark(bookmark);
    }
  }
  ...
  function toggleLoop() {
    if (snapshot.loop) player.setLoop();
      ...
      player.setLoop(activeCue.start + delay, activeCue.end + delay);
      void player.play();
    }
  }
  ...
  async function removeSaved() {
    ...
    try {
      ...
      if (alive)
        storageError = 'Saved audiobook data could not be removed. Close other tabs and try again.';
    } finally {
      if (alive) {
        player.setRate(1);
        clearing = false;
...
<Sheet.Root bind:open>
  <Sheet.Content
    side="bottom"
    class="audiobook-sheet max-h-[85dvh] overflow-y-auto [writing-mode:horizontal-tb]"
  ...
  >
    ...
    <Sheet.Description
      >Listen alongside {bookTitle} using local audio and timed subtitles. Files are not uploaded.</Sheet.Description
    >
      ...
      {#if cues.length}
        ...
        <div class="matching">
          ...
          <p class="hint">
            ...
            in the book first. Unmatched cues still play. After changing reader layout, match again.
          </p>
