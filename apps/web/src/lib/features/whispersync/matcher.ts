...
const ignored = [...ignoredRubyElements, 'script', 'style', 'template', 'noscript', '[hidden]', '[aria-hidden="true"]'].join(',')
...
export function normalizeText(value: string): string {
  ...
  return value.normalize('NFKC').toLowerCase().replace(/ς/g, 'σ').replace(/[\p{P}\p{Z}\s]/gu, '')
}

export interface TextSpan { node: Text; original: string; start: number; end: number }
...
export interface CueMatch { start: number; end: number; score: number; approximate?: boolean }
...
export async function buildBookIndex(root: HTMLElement, signal?: AbortSignal, onInvalidated?: () => void): Promise<BookIndex> {
  if (!segmenter) throw new Error('This browser cannot segment book text safely. Transcript playback is still available.')
  ...
  const changed = (records: MutationRecord[]) => records.some((record) =>
    record.type !== 'attributes' || record.target !== root || record.attributeName !== 'aria-hidden'
  )
  const observer = new Observer((records) => { if (changed(records)) invalidate() })
  ...
  observer.observe(root, { subtree: true, childList: true, characterData: true, attributes: true, attributeFilter: ['hidden', 'aria-hidden'] })
  const isCurrent = () => {
    if (changed(observer.takeRecords()) || !root.isConnected) invalidate()
    return valid && !disposed
  }
  const dispose = () => { disposed = true; valid = false; observer.disconnect() }
  const assertCurrent = () => {
    check(signal)
  ...
  try {
    ...
    while ((node = walker.nextNode())) {
      check(signal)
      visited += 1
      if (visited > MAX_BOOK_NODES) throw new Error('Book contains too many nodes for audiobook matching')
      if (node.nodeType === 3) {
        const text = node as Text
        const original = text.data
        if (rawLength + original.length > MAX_BOOK_RAW_UNITS) throw new Error('Book is too large for audiobook matching (8 million source characters)')
        if (original.length) {
      ...
      if (visited % 256 === 0) { await yieldToReader(signal); assertCurrent() }
    }
    ...
    for (const { segment, index } of segmenter.segment(raw)) {
      ...
      if (required > MAX_BOOK_UNITS) throw new Error('Book is too large for audiobook matching (4 million normalized characters)')
      if (required > starts.length) {
        const capacity = Math.min(MAX_BOOK_UNITS, Math.max(required, starts.length * 2))
        const nextStarts = new Uint32Array(capacity), nextEnds = new Uint32Array(capacity)
      ...
      for (let i = length; i < required; i += 1) { starts[i] = index; ends[i] = index + segment.length }
      length = required
      budget += segment.length
      if (budget >= 8192) { budget = 0; await yieldToReader(signal); assertCurrent() }
    }
    assertCurrent()
    return { root, text: parts.join(''), spans, starts: starts.slice(0, length), ends: ends.slice(0, length), isCurrent, dispose }
...
export function rangeForMatch(index: BookIndex, match: CueMatch): Range | undefined {
  if (!index.isCurrent() || !Number.isInteger(match.start) || !Number.isInteger(match.end) || match.end <= match.start || match.start < 0 || match.end > index.text.length) return undefined
...
export function offsetForSelection(index: BookIndex, range: Range): number | undefined {
  ...
  for (const span of index.spans) {
    if (span.node === range.startContainer) { sourceOffset = span.start + range.startOffset; break }
    ...
    if (candidate.comparePoint(range.startContainer, range.startOffset) < 0) { sourceOffset = span.start; break }
  }
...
export async function matchCues(text: string, cues: readonly Cue[], options: MatchOptions = {}): Promise<(CueMatch | undefined)[]> {
  if (options.start !== undefined && !Number.isInteger(options.start)) throw new Error('Invalid matching start offset')
  ...
  for (let i = 0; i < cues.length; i += 1) {
    ...
    if (at >= 0 && (characters >= 4 || (anchored && window.indexOf(needle, at + 1) < 0))) {
      ...
      for (let offset = 0; offset < Math.min(256, window.length - needle.length + 1); offset += 1) {
        const first = window.charCodeAt(offset), after = window.charCodeAt(offset + needle.length)
        ...
        if (score > 0.9) {
          const candidate = { start: cursor + offset, end: cursor + offset + needle.length, score, approximate: true }
          candidates.push(candidate)
          if (!match || score > match.score) match = candidate
        }
        if (offset % 64 === 63) await yieldToReader(options.signal)
      }
      if (match && candidates.some((candidate) => candidate.score >= match!.score - 0.01 && Math.abs(candidate.start - match!.start) > 2)) match = undefined
    }
    matches.push(match)
    if (match) { cursor = match.end; anchored = true }
    if (i % 32 === 31) {
...
export class ReaderHighlight {
  ...
  constructor(window: Window) { this.host = window as unknown as HighlightWindow }
  get supported(): boolean { return !!this.host.CSS?.highlights && !!this.host.Highlight }
  ...
  clear(): void {
    ...
    if (registry && this.value && registry.get(this.name) === this.value) registry.delete(this.name)
