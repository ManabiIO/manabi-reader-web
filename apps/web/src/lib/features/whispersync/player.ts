...
export interface AudioIdentity { name: string; size: number; lastModified: number }
...
export class LocalAudioPlayer {
  ...
  constructor(private readonly environment: PlayerEnvironment, private readonly changed: (state: PlaybackState) => void) {}

  get snapshot(): PlaybackState {
    // timeupdate is not guaranteed immediately before pause, close or pagehide.
    const time = this.pendingSeek ?? (this.state.ready && this.audio ? finite(this.audio.currentTime, this.state.time) : this.state.time)
    return { ...this.state, time: Math.max(0, time), file: this.state.file && { ...this.state.file }, loop: this.state.loop && { ...this.state.loop } }
  }

  private publish(): void { if (!this.disposed) this.changed(this.snapshot) }

  load(file: File, resume?: { identity: string; position: number }): void {
    if (this.disposed) return
    this.release()
    const rate = this.state.rate
    this.state = { file: { name: file.name, size: file.size, lastModified: file.lastModified }, time: 0, duration: 0, ready: false, paused: true, rate }
    this.pendingSeek = resume?.identity === audioIdentity(file) ? Math.max(0, finite(resume.position)) : 0
    ...
    try {
      ...
      const listen = (type: string, callback: () => void) => {
        const guarded = () => { if (!this.disposed && this.audio === audio) callback() }
        audio.addEventListener(type, guarded)
        this.removeListeners.push(() => audio.removeEventListener(type, guarded))
      }
      listen('loadedmetadata', () => { this.metadataAvailable = true; this.updateDuration() })
      ...
      listen('seeking', () => {
        ...
        if (loop && (audio.currentTime < loop.start || audio.currentTime >= loop.end)) this.clearLoop()
        this.update()
      })
      listen('play', () => { this.state.error = undefined; this.update(); this.scheduleFrame() })
      listen('pause', () => { this.playIntent += 1; this.cancelFrame(); this.update() })
      listen('ratechange', () => {
      ...
      listen('ended', () => {
        if (this.state.loop) { this.seek(this.state.loop.start); void this.play() }
        else { this.cancelFrame(); this.update() }
      })
      listen('error', () => {
        this.playIntent += 1
        ...
        this.clearLoop()
        audio.pause()
        this.state.error = 'This audio file could not be played. Try a browser-supported codec or another local file.'
  ...
  private updateDuration(): void {
    ...
    if (this.state.loop && (!this.state.ready || this.state.loop.end > this.state.duration)) this.clearLoop()
    this.update()
  }
  ...
  private update(): void {
    ...
    if (loop && !audio.paused && !audio.seeking && audio.currentTime >= loop.end) {
      try { audio.currentTime = loop.start } catch { this.clearLoop() }
    }
  ...
  async play(): Promise<void> {
    ...
    try {
      ...
      if (audio === this.audio && intent === this.playIntent && !this.disposed) {
        this.state.error = 'Playback was not allowed or the audio could not be decoded. Use the audio Play button to try again.'
        this.update()
      }
    }
  }

  pause(): void { this.playIntent += 1; this.audio?.pause(); this.cancelFrame(); this.update() }

  seek(time: number): void {
    if (!this.audio || this.disposed || this.mediaFailed || !Number.isFinite(time)) return
    const requested = Math.max(0, time)
    if (!this.state.ready) { this.pendingSeek = requested; this.state.time = requested; this.publish(); return }
    const target = Math.min(this.state.duration, requested)
    const loop = this.state.loop
    if (loop && (target < loop.start || target >= loop.end)) this.clearLoop()
    try { this.audio.currentTime = target; this.update() }
    catch { this.state.error = 'Seeking is not available for this audio file yet.'; this.publish() }
  }
  ...
  setLoop(start?: number, end?: number): void {
    if (this.disposed) return
    this.clearLoop()
    if (this.audio && this.state.ready && start !== undefined && end !== undefined && Number.isFinite(start) && Number.isFinite(end)) {
  ...
  private clearLoop(): void { this.state.loop = undefined }

  private scheduleFrame(): void {
    if (!this.audio || this.audio.paused || this.mediaFailed || this.frame !== undefined || this.disposed) return
  ...
  private release(): void {
    ...
    try { previous?.pause() } catch { /* Continue releasing owned resources. */ }
    try { previous?.removeAttribute('src'); previous?.load() } catch { /* Likewise. */ }
    try { this.environment.attach(undefined) } catch { /* Host may already be gone. */ }
