  ...
  itemRefs.forEach((item, spineIndex) => {
    ...
    if (importHTMLFixMode === ImportHTMLFixMode.EXTENDED) {
      contentToParse = contentToParse
        ...
        .replace(htmlHexEntitiesRegex, (_, hex) => decodeEpubNumericEntity(hex, 16))
        .replace(htmlDecEntitiesRegex, (_, dec) => decodeEpubNumericEntity(dec, 10))
...
function flattenAnchorHref(el: HTMLElement) {
  Array.from(el.getElementsByTagName('a')).forEach((tag) => {
    ...
    if (!oldHref || !isSafeEpubInternalHref(oldHref)) return;
    ...
    tag.setAttribute(EPUB_SOURCE_HREF_ATTRIBUTE, oldHref);
    tag.setAttribute('href', legacyFlattenedEpubHref(oldHref));
  });
}
