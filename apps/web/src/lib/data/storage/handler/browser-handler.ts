...
import { updateBookLastRead } from '$lib/data/database/books-db/book-records';
...
export class BrowserStorageHandler extends BaseStorageHandler {
  ...
  async updateLastRead(book: BooksDbBookData) {
    const { id, lastBookOpen = 0 } = book;
    const current = await updateBookLastRead(await database.db, id, lastBookOpen);
    if (!current) return;
