...
function copyAsset(asset, purpose) {
  if (
    ...
    !asset.revision ||
    !hash(asset.contentSha256)
  )
    throw new TypeError('Dictionary assets need immutable identity, revision and content checksum');
  if (typeof asset.source !== 'string' || !asset.source)
    throw new TypeError('Missing dictionary source');
  if (asset.purpose !== purpose)
    throw new TypeError('Processing dictionaries are not definition dictionaries');
  return Object.freeze(JSON.parse(canonicalJSON(asset)));
}
export function installationPlan(selectedDefinitions, processingAssets = []) {
  if (!Array.isArray(selectedDefinitions) || !Array.isArray(processingAssets))
    throw new TypeError('Invalid installation selection');
  if (selectedDefinitions.length === 0) return Object.freeze([]);
  const entries = new Map();
  for (const [assets, purpose] of [
    [processingAssets, 'processing'],
    [selectedDefinitions, 'definitions']
  ])
    for (const raw of assets) {
      const asset = copyAsset(raw, purpose),
        existing = entries.get(asset.id);
      if (existing && canonicalJSON(existing) !== canonicalJSON(asset))
        throw new Error('Conflicting dictionary identities');
...
function verifiedReceipt(receipt, asset) {
  if (
    !receipt ||
    receipt.id !== asset.id ||
    receipt.contentSha256 !== asset.contentSha256 ||
    receipt.purpose !== asset.purpose ||
    !id(receipt.artifactID) ||
    !hash(receipt.artifactSha256) ||
    typeof receipt.importerVersion !== 'string' ||
    !receipt.importerVersion ||
    typeof receipt.formatVersion !== 'string' ||
...
export class DictionaryInstallationCoordinator {
  constructor({ processingAssets = [], storage, report = () => {} }) {
    ...
    for (const method of ['withLock', 'ensure', 'activateProcessingSet', 'readProcessingSet'])
      if (typeof storage?.[method] !== 'function')
        throw new TypeError('Missing installation storage method: ' + method);
  ...
  get processingResourceCount() {
    return this.processingAssets.length;
  }
    ...
    return this.storage.withLock(
      async () => {
        ...
        const processing = plan.filter((a) => a.purpose === 'processing'),
          definitions = plan.filter((a) => a.purpose === 'definitions'),
          installed = [],
          receipts = [];
        ...
        try {
          ...
          aborted(signal);
          if (fingerprint !== null) {
            await this.storage.activateProcessingSet(
              { schema: 1, fingerprint, resources: receipts },
              { signal }
            );
            aborted(signal);
            ...
            if (
              ...
              (await processingSetFingerprint(active.resources)) !== fingerprint
            )
