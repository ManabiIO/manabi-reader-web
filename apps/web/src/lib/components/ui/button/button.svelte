<script lang="ts" module>
  ...
  export const buttonVariants = tv({
    base: "focus-visible:border-ring focus-visible:ring-ring/30 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive dark:aria-invalid:border-destructive/50 border border-transparent bg-clip-padding text-sm font-normal focus-visible:ring-3 aria-invalid:ring-3 [&_svg:not([class*='size-'])]:size-4 group/button inline-flex shrink-0 items-center justify-center min-w-0 max-w-full whitespace-normal text-center [overflow-wrap:anywhere] transition-[color,background-color,border-color,box-shadow] duration-150 outline-none select-none disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 pointer-coarse:min-h-11 pointer-coarse:min-w-11 [&_svg]:pointer-events-none [&_svg]:shrink-0",
    variants: {
      variant: {
        // Apple.com-style action hierarchy, using Manabi's own palette.
        // Keep hover fills opaque so their contrast does not depend on the canvas.
        default:
          'bg-primary text-primary-foreground hover:bg-[color-mix(in_srgb,var(--primary),var(--primary-foreground)_8%)]',
        outline:
          'border-primary bg-transparent text-primary hover:border-primary hover:bg-primary hover:text-primary-foreground aria-expanded:border-primary aria-expanded:bg-primary aria-expanded:text-primary-foreground',
        secondary:
