...
<header
  class="app-header fixed inset-x-0 top-0 z-10 border-b border-border bg-card text-foreground"
...
>
  ...
  <div class="mx-auto flex min-h-14 max-w-7xl items-center gap-2 overflow-x-auto px-3 pb-2 sm:px-6">
    <div role="group" aria-label="Statistics view" class="flex gap-1">
      <Button
        variant={$lastStatisticsTab$ === StatisticsTab.SUMMARY ? 'secondary' : 'ghost'}
        aria-pressed={$lastStatisticsTab$ === StatisticsTab.SUMMARY}
        onclick={() => ($lastStatisticsTab$ = StatisticsTab.SUMMARY)}>Summary</Button
      >
      <Button
        variant={$lastStatisticsTab$ === StatisticsTab.OVERVIEW ? 'secondary' : 'ghost'}
