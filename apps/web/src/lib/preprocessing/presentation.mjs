...
export class ChapterOpeningController {
  constructor({session, stage, ready = () => {}, report = () => {}, isVisible = () => true}) {
    this.session = session; this.stage = stage; this.ready = ready;
    this.report = report; this.isVisible = isVisible;
    this.ticket = 0; this.disposed = false; this.current = null;
  }
  async open(index) {
    if (this.disposed) throw new DOMException('Reader closed', 'AbortError');
    const ticket = ++this.ticket;
    let staged = null, unmount = null;
    const check = () => {
      if (this.disposed || ticket !== this.ticket) throw new DOMException('Superseded', 'AbortError');
    };
    try {
      ...
      if (result.sidecar !== null) {
        ...
        if (typeof unmount !== 'function') throw new Error('Preprocessor mount must return a disposer');
      }
      check();
      ...
      staged.commit();
      this.current = {stage: staged, unmount}; staged = null; unmount = null;
      ...
      try { previous?.unmount?.(); } catch { /* committed successor remains */ }
      try { previous?.stage.rollback(); } catch { /* committed successor remains */ }
      try { this.ready({index, revision, hasAnnotations: result.sidecar !== null}); } catch { /* observers do not own publication */ }
      this.session.didPresent(revision, {isVisible: this.isVisible});
      return result;
    } catch (error) {
      try { unmount?.(); } catch { /* preserve original opening error */ }
      try { staged?.rollback?.(); } catch { /* preserve original opening error */ }
      if (!this.disposed && ticket === this.ticket) {
        try { this.report({type: 'opening-failed', name: error?.name ?? 'Error'}); } catch {}
      }
      throw error;
    }
  }
  dispose() {
    ...
    this.disposed = true; ++this.ticket; this.session.dispose();
    try { this.current?.unmount?.(); } finally { this.current?.stage.rollback(); this.current = null; }
  }
}
...
export function chapterForResume(sectionCounts, exploredCharCount) {
  ...
  for (let i = 0; i < sectionCounts.length; ++i) {
    total += sectionCounts[i]; if (!Number.isSafeInteger(total)) throw new Error('Character count overflow');
