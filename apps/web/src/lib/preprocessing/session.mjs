...
export class BookPreprocessingSession {
  ...
  assertLive() {
    if (this.disposed) throw new globalThis.DOMException('Book closed', 'AbortError');
  }
    ...
    return this.exclusive(async () => {
      ...
      if (cached) {
        try {
          ...
          if (result.sidecar === null) result = null;
          else cacheHit = true;
        } catch {
  ...
  async open(index) {
    ...
    if (revision !== this.revision)
      throw new globalThis.DOMException('Superseded chapter', 'AbortError');
