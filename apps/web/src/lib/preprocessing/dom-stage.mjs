...
export function createDOMChapterStage({
  container,
  sanitize,
  configure = () => {},
  bindResources = async () => () => {},
  ...
  if (
    ...
    typeof bindResources !== 'function' ||
    typeof restore !== 'function'
  )
    throw new Error('Incomplete chapter stage composition');
  return async (result, { signal, index, revision }) => {
    ...
    const diagnostic = (error) => {
      try {
        report({ type: 'stage-cleanup-failed', name: error?.name ?? 'Error' });
      } catch {
    ...
    const live = () => {
      check(signal);
      if (disposed) throw new globalThis.DOMException('Chapter stage disposed', 'AbortError');
    };
    try {
      ...
      root.innerHTML = html;
      revealStyle = new Map(
        ['position', 'inset', 'width', 'visibility', 'opacity', 'pointer-events', 'transition'].map(
          (name) => [
            name,
      ...
      return {
        ...
        async prepare() {
          live();
          if (prepared) return;
          if (preparing) throw new Error('Concurrent stage preparation');
          preparing = true;
          let cleanup = null;
          try {
            cleanup = await bindResources(root, { signal, index, revision, result });
            if (typeof cleanup !== 'function')
              throw new Error('Resource binder must return a disposer');
            live();
            releaseResources = cleanup;
            cleanup = null;
            await restore(root, { signal, index, revision, result });
            live();
            prepared = true;
          } catch (error) {
            try {
        ...
        commit() {
          live();
          if (!prepared) throw new Error('Cannot reveal an unprepared chapter');
          committed = true;
          signal?.removeEventListener('abort', abortStage);
          for (const [name, [value, priority]] of revealStyle)
            root.style.setProperty(name, value, priority);
          root.inert = false;
          root.removeAttribute('aria-hidden');
          root.dataset.readerChapter = 'visible';
        },
        rollback
      };
    } catch (error) {
