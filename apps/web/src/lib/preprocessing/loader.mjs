...
export function deploymentURL(value, base) {
  ...
  const url = new globalThis.URL(value, base),
    origin = new globalThis.URL(base);
  if (
...
export async function loadDeploymentManifest({
  ...
  const manifest = JSON.parse(
    new globalThis.TextDecoder('utf-8', { fatal: true }).decode(
      await boundedBytes(response, 65536, signal)
    )
  );
...
export async function loadDeploymentPreprocessor({
  ...
  const objectURL = globalThis.URL.createObjectURL(
    new globalThis.Blob([bytes], { type: 'text/javascript' })
  );
  let candidate = null;
  try {
