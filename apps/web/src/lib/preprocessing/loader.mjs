...
export function deploymentURL(value, base) {
  ...
  if (url.origin !== origin.origin || url.username || url.password || url.hash || url.search ||
      !['http:', 'https:'].includes(url.protocol) || /%2f|%5c/i.test(url.pathname)) {
...
async function boundedBytes(response, maximum, signal) {
  ...
  try {
    aborted(signal);
    while (true) {
      const {value, done} = await reader.read(); aborted(signal); if (done) break;
      if (!(value instanceof Uint8Array)) throw new Error('Invalid preprocessor response stream');
      size += value.byteLength;
      if (!Number.isSafeInteger(size) || size > maximum) throw new Error('Preprocessor download exceeds budget');
 ...
 */
export async function loadDeploymentManifest({manifestURL, baseURL, fetcher = fetch, signal} = {}) {
  aborted(signal);
  if (!manifestURL) return {manifest: null, assetBaseURL: null, status: 'not-configured'};
  const url = deploymentURL(manifestURL, baseURL);
  const response = await fetcher(url, {signal, credentials: 'same-origin', cache: 'no-cache', redirect: 'error'});
  aborted(signal);
  if (response.status === 404) return {manifest: null, assetBaseURL: null, status: 'not-installed'};
  if (!response.ok) throw new Error(`Preprocessor manifest HTTP ${response.status}`);
  const manifest = JSON.parse(new TextDecoder('utf-8', {fatal: true}).decode(await boundedBytes(response, 65536, signal)));
  ...
  if (!/^[a-f0-9]{64}$/.test(manifest.entry?.sha256 ?? '')) throw new Error('Missing module integrity');
  if (manifest.processingResources !== undefined && !Array.isArray(manifest.processingResources)) throw new Error('Invalid processing catalogue');
...
export async function loadDeploymentPreprocessor({manifestURL, baseURL, fetcher = fetch,
  importer = (url) => import(/* @vite-ignore */ url), report = () => {}, signal} = {}) {
  ...
  if (await sha256(bytes) !== manifest.entry.sha256) throw new Error('Preprocessor module integrity mismatch');
  aborted(signal);
  ...
  try {
    ...
    aborted(signal);
    if (typeof module?.createPreprocessor !== 'function') throw new Error('Missing preprocessor factory');
    ...
    if (typeof candidate?.dispose === 'function') {
      ...
      catch { try { report({type: 'plugin-cleanup-failed'}); } catch {} }
    }
    throw error;
