...
class LocksDouble {
  ...
  pump() {
    ...
    this.held = true;
    Promise.resolve()
      .then(() => e.callback({ name: 'manabi-moss-inference', mode: 'exclusive' }))
      .then(
        (value) => {
          this.held = false;
          this.releases++;
...
async function harness(body, withLocks = true) {
  const before = Object.getOwnPropertyDescriptor(navigator, 'locks'),
    oldRange = globalThis.IDBKeyRange;
  const locks = new LocksDouble();
  Object.defineProperty(navigator, 'locks', {
    configurable: true,
    value: withLocks ? locks : undefined
  });
  ...
  try {
    ...
    await store.close();
    if (before) Object.defineProperty(navigator, 'locks', before);
    else delete navigator.locks;
    if (oldRange === undefined) delete globalThis.IDBKeyRange;
    else globalThis.IDBKeyRange = oldRange;
  }
}
test('one origin lease spans a warm batch and its asynchronous model shutdown', () =>
  harness(async ({ queue, store, locks, errors }) => {
    ...
    function engine(name) {
      ...
      return {
        ...
        async transcribe() {
          if (name === 'A' && ++callsA === 1) await first.promise;
          return '[0][S01]テスト。[1]';
        },
        async dispose() {
          if (!loaded) return;
    ...
    const a = queue('account:a', engine('A')),
      b = queue('account:b', engine('B'));
    try {
      ...
      unload.resolve();
      await eventually(
        async () => (await store.local('account:b', 'jobs', j3.id)).status === 'complete'
      );
...
test('same-account batches do not claim jobs whose local File belongs to another workspace', () =>
  harness(async ({ queue, store, locks }) => {
    const first = deferred();
    ...
    try {
      ...
      await eventually(async () =>
        ['complete', 'failed'].includes((await store.local('guest', 'jobs', j2.id)).status)
      );
...
test('closing a queue waiting for the runtime lock never creates a model', () =>
  harness(async ({ queue, locks }) => {
...
test('all queue Close callers wait for the same runtime retirement', () =>
  harness(async ({ queue }) => {
    const transcribing = deferred(),
      retiring = deferred();
    let stops = 0,
      started = false;
    const q = queue('guest', {
    ...
    await q.enqueue(key, 'ja', '1', 2);
    await eventually(() => started);
    const first = q.dispose(),
      second = q.dispose();
    let done = false;
    void second.then(() => (done = true));
    try {
      transcribing.resolve('[0][S01]Test[1]');
      await eventually(() => stops === 1);
      await tick();
...
test('without Web Locks a drained workspace still releases its own model', () =>
...
test('model lifetime ends after a failed decode and the next explicit job can retry', () =>
  harness(async ({ queue, store }) => {
    let loaded = false,
      loads = 0,
      first = true;
...
test('a job admitted during retirement starts a fresh leased batch without being stranded', () =>
  harness(async ({ queue, store, locks }) => {
    ...
    try {
      ...
      retiring.resolve();
      await eventually(
        async () => (await store.local('guest', 'jobs', next.id)).status === 'complete'
      );
      await eventually(() => !locks.held);
...
test('closing during retirement does not restart a newly admitted batch', () =>
  harness(async ({ queue, store, locks }) => {
    const retiring = deferred();
    let loaded = false,
      stopping = false,
...
class WorkerDouble extends EventTarget {
  ...
  reply(type, id, value = null) {
    this.dispatchEvent(new MessageEvent('message', { data: { type, id, value } }));
  }
}
async function clients(body) {
  ...
  try {
    ...
    for (const w of WorkerDouble.all) {
      ...
      if (request) w.reply('disposed', request.id);
    }
    await stopped;
...
async function prepared(client) {
  const p = client.prepare(new AbortController().signal, () => {});
  await eventually(() => WorkerDouble.all.at(-1)?.calls.some((c) => c.type === 'prepare'));
  const w = WorkerDouble.all.at(-1);
  w.reply('ready', w.calls[0].id);
  await p;
  return w;
}
test('a replacement client worker waits for previous pool shutdown acknowledgement', () =>
  clients(async (client) => {
    const old = await prepared(client),
      stopped = client.dispose();
    let done = false;
...
test('cancelled preparation waiting for shutdown cannot create a replacement worker', () =>
  clients(async (client) => {
    const old = await prepared(client);
    client.dispose();
    const c = new AbortController();
    const p = client.prepare(c.signal, () => {}),
      rejected = assert.rejects(p, { name: 'AbortError' });
    c.abort();
    await rejected;
    old.reply('disposed', old.calls.find((c) => c.type === 'dispose').id);
    await tick();
...
test('a second explicit Close fences a pending preparation after shutdown', () =>
  clients(async (client) => {
    const old = await prepared(client);
    client.dispose();
    const p = client.prepare(new AbortController().signal, () => {}),
...
test('unresponsive worker shutdown has a bounded termination fallback', () =>
  clients(async (client) => {
