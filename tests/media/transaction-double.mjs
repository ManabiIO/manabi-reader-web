...
export class TransactionFactory {
  ...
  releaseOpen() {
    for (const fn of this.openings.splice(0)) fn();
  }
  open() {
    const request = {};
    // eslint-disable-next-line @typescript-eslint/no-this-alias -- Keep the factory distinct from the mock connection receiver.
    const factory = this;
    const db = {
      closed: false,
      ...
      transaction(name, mode) {
        ...
        const transaction = {
          ...
          objectStore(storeName = names[0]) {
            ...
            return {
              get(key) {
                factory.reads.push({ store: storeName, operation: 'get', key });
                return enqueue(() => structuredClone(values().get(key)));
              },
              getAll(query) {
                factory.reads.push({ store: storeName, operation: 'getAll', query });
                return enqueue(() =>
                  [...values().entries()]
                    .filter(([key]) => !query || (key >= query.lower && key <= query.upper))
              ...
              put(value, key) {
                ...
                if (++factory.putCount === factory.failPutNumber)
              ...
              delete(key) {
                return enqueue(() => values().delete(key));
              }
            };
          }
        };
        factory.queue = factory.queue.then(async () => {
          tables = new Map(
            names.map((n) => [n, structuredClone(factory.tables.get(n) ?? new Map())])
          );
          ...
          if (aborted) transaction.onabort?.();
            ...
            if (mode === 'readwrite') names.forEach((n) => factory.tables.set(n, tables.get(n)));
    ...
    if (this.holdOpen) this.openings.push(finish);
