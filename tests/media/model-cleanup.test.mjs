...
const bounded = async promise => {
    let timer;
    try { return await Promise.race([promise, new Promise((_, reject) => { timer = setTimeout(() => reject(Error('Operation remained blocked')), 500); })]); }
    finally { clearTimeout(timer); }
};

test('writer abort that never settles does not hide HTTP failure', {timeout: 1500}, async () => {
    let aborted = 0;
    const out = target({abort() { aborted++; return new Promise(() => {}); }});
    await assert.rejects(bounded(downloadVerified(new Response(null, {status: 503}), out, expected, signal())), /Model download failed \(503\)/);
    assert.equal(aborted, 1); assert.equal(out.closed, false);
});

test('Cancel detaches a stalled disk write and never closes or reports late progress', {timeout: 1500}, async () => {
    const controller = new AbortController(), started = deferred(), disk = deferred(), updates = [];
    const out = target({write() { this.writes++; started.resolve(); return disk.promise; }, abort() { this.aborted = true; return new Promise(() => {}); }});
    const result = downloadVerified(new Response(payload), out, expected, controller.signal, p => updates.push(p));
    const rejected = assert.rejects(bounded(result), {name: 'AbortError'});
    await started.promise; controller.abort(); await rejected;
    disk.resolve(); await new Promise(r => setTimeout(r, 0));
    assert.equal(out.writes, 1); assert.equal(out.aborted, true); assert.equal(out.closed, false); assert.deepEqual(updates, []);
});

test('late rejection of an abandoned model write remains observed', {timeout: 1500}, async () => {
    const controller = new AbortController(), started = deferred(), disk = deferred();
    const out = target({write() { started.resolve(); return disk.promise; }});
    const result = downloadVerified(new Response(payload), out, expected, controller.signal);
    const rejected = assert.rejects(bounded(result), {name: 'AbortError'});
    await started.promise; controller.abort(); await rejected;
    disk.reject(Error('Late disk failure')); await new Promise(r => setTimeout(r, 0));
    assert.equal(out.closed, false); assert.equal(out.aborted, true);
});

test('late Cancel during verified close does not report download success', {timeout: 1500}, async () => {
    const controller = new AbortController(), closing = deferred(), commit = deferred();
    const out = target({close() { closing.resolve(); return commit.promise; }});
    const result = downloadVerified(new Response(payload), out, expected, controller.signal);
    const rejected = assert.rejects(bounded(result), {name: 'AbortError'});
    await closing.promise; controller.abort(); await rejected;
    ...
    commit.resolve(); await new Promise(r => setTimeout(r, 0));
    assert.equal(out.aborted, true);
});

test('read cancellation cannot leave the pipeline blocked by a noncooperating reader', {timeout: 1500}, async () => {
    const controller = new AbortController(), started = deferred(), reading = deferred();
    let released = 0, canceled = 0;
    ...
    const response = {ok: true, status: 200, headers: new Headers(), body: {getReader() { return {
        read() { started.resolve(); return reading.promise; }, cancel() { canceled++; return new Promise(() => {}); }, releaseLock() { released++; }
    }; }}};
    const out = target(), result = downloadVerified(response, out, expected, controller.signal);
    const rejected = assert.rejects(bounded(result), {name: 'AbortError'});
    await started.promise; controller.abort(); await rejected;
    reading.reject(Error('Late transport failure')); await new Promise(r => setTimeout(r, 0));
    assert.ok(canceled > 0); assert.equal(released, 1); assert.equal(out.writes, 0); assert.equal(out.closed, false);
});
