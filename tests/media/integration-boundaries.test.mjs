...
test('cloud connections use the exact existing backend items envelope', () => {
  ...
  for (const value of [
    ...
    assert.throws(() => connectionsFrom(value));
});
test('connection validation rejects malformed IDs, roots, and reconnect state', () => {
  for (const patch of [
    ...
    assert.throws(() => connectionsFrom({ items: [{ ...connection, ...patch }] }));
});
test('folder entry validation rejects invalid sizes, types and control characters', () => {
  for (const patch of [
    ...
    assert.throws(() => listingFrom({ items: [{ ...entry('one'), ...patch }], cursor: '' }));
...
test('folder pagination checks all cursor cycles including empty filtered pages', async () => {
  ...
  const transport = {
    ...
    isCurrent: () => true,
    async request() {
      return { items: [], cursor: ['a', 'b', 'a'][calls++] };
    }
  };
  await assert.rejects(
    listCloudFolder(transport, connection, 'selected', 'selected', signal()),
    /repeated a cursor/
  );
...
test('folder pagination cannot run forever on distinct empty cursors', async () => {
  ...
  const transport = {
    ...
    isCurrent: () => true,
    async request() {
      return { items: [], cursor: 'page-' + ++calls };
    }
  };
  await assert.rejects(
    listCloudFolder(transport, connection, 'selected', 'selected', signal()),
...
test('folder pages reject repeated provider IDs instead of importing twice', async () => {
  ...
  const transport = {
    ...
    isCurrent: () => true,
    async request() {
      return { items: [entry('one')], cursor: calls++ ? '' : 'next' };
    }
  };
  await assert.rejects(
    listCloudFolder(transport, connection, 'selected', 'selected', signal()),
    /changed while listing/
  );
});
...
test('late cloud responses lose authority on account change', async () => {
  const d = deferred();
  let current = true;
  const transport = { userId: 'u', isCurrent: () => current, request: () => d.promise };
  const request = cloudRequest(transport, 'connections/', signal());
  current = false;
  d.resolve({ items: [] });
...
test('late cloud responses are discarded after closing the picker', async () => {
  ...
  const request = cloudRequest(
    { userId: 'u', isCurrent: () => true, request: () => d.promise },
    'connections/',
    controller.signal
  );
  controller.abort();
...
test('cloud pagination retains selected root and opaque parent', async () => {
  ...
  const transport = {
    ...
    async request(path, options) {
      const url = new URL(path, 'https://manabi.io');
      assert.equal(url.searchParams.get('parent'), 'id:opaque/parent');
      assert.equal(url.searchParams.get('root'), 'selected');
      assert.equal(options.userId, 'u');
      return { items: [entry('item' + count)], cursor: count++ ? '' : 'next' };
    }
  };
  assert.equal(
    (await listCloudFolder(transport, connection, 'selected', 'id:opaque/parent', signal())).length,
    2
  );
});
function pipeline(input, outer) {
  let disposed = 0;
  const instance = new MediaPipeline(
    {
      create() {
        return {
          ...input,
  ...
  return { instance, disposed: () => disposed };
}
test('decode cancellation covers unresolved track discovery', async () => {
  const d = deferred(),
    c = new AbortController(),
    h = pipeline({
      getAudioTracks: () => d.promise,
      dispose() {
        d.reject(new DOMException('Aborted', 'AbortError'));
      }
    });
  const job = h.instance.decode(1, 0, 1, c.signal);
  c.abort();
...
test('decode cancellation covers slow codec capability checks', async () => {
  const d = deferred(),
    c = new AbortController(),
    h = pipeline({ getAudioTracks: async () => [{ id: 1, canDecode: () => d.promise }] });
  const job = h.instance.decode(1, 0, 1, c.signal);
...
test('invalid decode ranges fail before metadata or audio allocation', async () => {
  ...
  for (const [start, end] of [
    ...
    await assert.rejects(h.instance.decode(1, start, end, signal()), /bounded windows/);
  h.instance.dispose();
});
...
test('invalid video metadata fails before it is saved as library state', async () => {
  const h = pipeline({
    computeDuration: async () => Infinity,
    getPrimaryVideoTrack: async () => null
  });
  await assert.rejects(h.instance.metadata(), /Invalid video metadata/);
  h.instance.dispose();
});
...
function queueDoubleTransactions(store) {
  store.enqueueJob = async (scope, draft, guard = () => {}) => {
    const rows = await store.listLocal(scope, 'jobs');
    guard();
    const old = rows.find(
      (j) =>
  ...
  store.updateLocal = async (s, k, id, fn) => {
    const old = await store.local(s, k, id),
      next = fn(old);
    if (next !== old) await store.putLocal(s, k, id, next);
...
test('closing while the queue reads pending jobs never starts an orphan worker', async () => {
  ...
  const store = {
    async listLocal() {
      return ++reads === 1 ? [] : d.promise;
    },
    async putLocal(s, k, id, v) {
      jobs.push(structuredClone(v));
    }
  };
...
test('closing before a delayed enqueue check prevents the new durable job', async () => {
  ...
  const adding = queue.enqueue(key, 'en', '1', 1);
  await queue.dispose();
  d.resolve([]);
...
test('queue disposal drains the active paused checkpoint before storage closes', async () => {
  ...
  const store = {
    ...
    async putLocal(s, k, id, v) {
      latest = structuredClone(v);
      if (hold && v.status === 'paused') await persist.promise;
    }
  };
  const engine = {
    prepare(signal) {
...
test('a canceled lock waiter cannot overwrite another tab completed job', async () => {
  const old = Object.getOwnPropertyDescriptor(navigator, 'locks'),
    wait = deferred();
  let latest,
    writes = 0;
  ...
  try {
    ...
    if (old) Object.defineProperty(navigator, 'locks', old);
...
test('restricted IndexedDB getter fails lazily and does not break construction', async () => {
  ...
  try {
    ...
    await assert.rejects(store.local('guest', 'x', 'x'), /unavailable/);
    await store.close();
  } finally {
...
test('closed media store cannot be reopened by a late asynchronous continuation', async () => {
  ...
  await store.close();
  await assert.rejects(store.records('guest'), /closed/);
...
test('an existing complete target-language caption suppresses bulk generation', () => {
  assert.equal(
    missingTranscriptDecision(
      [{ language: 'ja-JP', complete: true, forced: false }],
      'ja',
      'unsupported'
    ),
    'present'
...
test('forced-only and translated captions do not count as full original captions', () => {
  assert.equal(
    missingTranscriptDecision(
      [
        { language: 'ja', complete: true, forced: true },
        { language: 'en', complete: true, forced: false }
      ],
      'ja',
      'complete'
    ),
...
import {
  validateCloudLocator,
  cloudInfoPath
} from '../../.cache/media-test-build/cloud-locator.js';
...
test('device cloud locators contain no credential or download capability fields', () => {
  ...
  for (const extra of [{ url: 'https://capability' }, { token: 'secret' }, { version: 'digest' }])
    assert.throws(() => validateCloudLocator({ ...locator, ...extra }));
  assert.equal(
    new URL(cloudInfoPath(locator), 'https://manabi.io').searchParams.get('root'),
    locator.root
  );
});
test('cloud locators cannot alter the selected-root API path', () => {
  for (const value of [
    null,
    { ...locator, connectionId: '../../escape' },
    { ...locator, root: '' },
    { ...locator, id: '\nheader' }
  ])
    assert.throws(() => cloudInfoPath(value));
});
test('cloud source validates owner, revision, exact endpoint and immutable metadata', () => {
  ...
  try {
    const version = 'b'.repeat(64),
      url =
        `/api/reader-web/connections/${connection.id}/media/?` +
        new URLSearchParams({ root: locator.root, id: locator.id, user: 'u', version });
    const manifest = { name: 'video.mp4', size: 10, version, url };
    const source = cloudSource(manifest, 'u', () => true);
    manifest.size = 999;
    manifest.name = 'changed';
    assert.equal(source.size, 10);
    assert.equal(source.name, 'video.mp4');
    assert.deepEqual(source.cloud, locator);
    for (const bad of [
      url + '&user=u',
      url.replace('user=u', 'user=v'),
      url.replace('/media/', '/file/'),
      url + '#fragment',
      'https://other.invalid' + url
    ])
      assert.throws(() => cloudSource({ ...manifest, size: 10, url: bad }, 'u', () => true));
    assert.throws(() =>
      cloudSource({ ...manifest, size: 10, version: 'c'.repeat(64), url }, 'u', () => true)
    );
  } finally {
...
test('CPU runtime recipe disables dynamic JavaScript and pins the ggml submodule', () => {
  ...
  assert.match(source, /-sDYNAMIC_EXECUTION=0/);
  assert.match(source, /ggml_head != GGML_PIN/);
  assert.match(source, /LICENSE-MOSS.txt/);
  assert.match(source, /LICENSE-GGML.txt/);
});
