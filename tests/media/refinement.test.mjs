...
import {
  ...
  MOSS_WEB_ABI
} from '../../.cache/media-test-build/moss-runtime-contract.js';
...
test('MOSS bracketed numbers inside speech do not become spurious end timestamps', () => {
  const result = parseMoss(
    '[0.2][S01]Read [2026] and [1.5] aloud.[3.2][3.3][S02]次の文です。[5]',
    6
  );
  assert.deepEqual(
    result.map((c) => c.text),
    ['Read [2026] and [1.5] aloud.', '次の文です。']
  );
  assert.deepEqual(
    result.map((c) => [c.start, c.end]),
    [
      [0.2, 3.2],
      [3.3, 5]
    ]
...
test('MOSS parser accepts reference decimal forms and surrounding whitespace', () => {
  ...
  assert.equal(result[0].start, 0.25);
  assert.equal(result[0].end, 2);
});
test('malformed MOSS tail cannot silently publish earlier valid output', () => {
  for (const raw of ['[0][S01]One.[1]trailing', '[0][S01]One.[1][2][S02]Incomplete'])
    assert.throws(() => parseMoss(raw, 5));
});
test('nonempty MOSS speech with no usable interval fails instead of disappearing', () => {
  for (const raw of ['[1][S01]Spoken.[1]', '[5][S01]Spoken.[6]'])
    assert.throws(() => parseMoss(raw, 5), /interval/);
...
test('Japanese bracketed transcript text survives ordinary subtitle export/import', () => {
  ...
  for (const format of ['srt', 'vtt'])
    assert.deepEqual(
      parseSubtitles(serializeSubtitles({ cues }, format)).map((c) => c.text),
      cues.map((c) => c.text)
    );
});
const runtime = (changes = {}) => ({
  _moss_web_abi_version: () => MOSS_WEB_ABI,
  _moss_web_engine_revision: () => 16,
  _moss_web_ggml_revision: () => 32,
  UTF8ToString: (p) => (p === 16 ? MOSS.engineRevision : MOSS.ggmlRevision),
  ...changes
});
test('compiled runtime compatibility identity must match both engine and ggml', () => {
  ...
  for (const value of [
    runtime({ _moss_web_abi_version: () => 0 }),
    runtime({ UTF8ToString: () => 'stale' }),
    runtime({ _moss_web_engine_revision: undefined }),
    runtime({ _moss_web_ggml_revision: () => 0 }),
    runtime({ _moss_web_engine_revision: () => -1 }),
    runtime({ _moss_web_ggml_revision: () => NaN })
  ])
    assert.throws(() => assertRuntimeIdentity(value), /outdated or incompatible/);
});
...
test('worker verifies compiled runtime before the model download', () => {
  ...
  assert.ok(
    src.indexOf('assertRuntimeIdentity(runtime!, data.threaded)') < src.indexOf('await getModel(')
  );
...
test('build recipe exports the pinned ABI and provenance handshake', () => {
  ...
  for (const fn of ['moss_web_abi_version', 'moss_web_engine_revision', 'moss_web_ggml_revision']) {
    assert.ok(build.includes('_' + fn));
    assert.ok(bridge.includes(fn));
  }
  assert.ok(build.includes(MOSS.engineRevision.split('+')[1]));
  assert.ok(build.includes(MOSS.ggmlRevision));
});
test('forced subtitle exports preserve the forced flag when rediscovered', () => {
  const name = exportName('Episode.mp4', { language: 'ja', forced: true });
  assert.equal(name, 'Episode.ja.forced.srt');
...
test('indexed translated overlaps agree with reference including long crossing cues', () => {
  const cues = Array.from({ length: 400 }, (_, i) => ({
    id: String(i),
    start: i * 0.25,
    end: i * 0.25 + (i % 9 === 0 ? 30 : 0.3),
    text: String(i)
  }));
  const timeline = new CueTimeline(cues);
  for (let i = 0; i < 800; i++) {
    const start = (i % 200) * 0.51,
      end = start + 0.42,
      delay = (i % 31) * 0.1 - 1.5;
    assert.deepEqual(
      timeline.overlaps(start, end, delay),
      cues.filter((c) => c.start + delay < end && c.end + delay > start)
    );
  }
  assert.deepEqual(timeline.overlaps(2, 2), []);
  assert.deepEqual(timeline.overlaps(NaN, 3), []);
  assert.deepEqual(timeline.overlaps(0, 3, Infinity), []);
});
test('rejected response cancellation does not become an unhandled rejection', async () => {
  ...
  const response = new Response(
    new ReadableStream({
      cancel() {
        canceled++;
        return Promise.reject(Error('transport cancel failed'));
      }
    })
  );
  const pending = boundedResponse(response, 10, controller.signal);
  controller.abort();
...
test('oversized response rejection does not wait for a non-settling transport cancel', async () => {
  ...
  await Promise.race([
    assert.rejects(pending, /Oversized/),
    new Promise((_, reject) => setTimeout(() => reject(Error('response cleanup stalled')), 200))
  ]);
  assert.equal(canceled, 1);
});
test('invalid response size rejected before acquiring stream reader', async () => {
  for (const max of [-1, NaN, Infinity, 1.2])
    await assert.rejects(
      boundedResponse(new Response('x'), max, new AbortController().signal),
      /size limit/
    );
});
test('cloud account change cancels the unopened response body', async () => {
  ...
  try {
    const source = cloudSource(
      {
        ...
        size: 10,
        version,
        url: `/api/reader-web/connections/${id}/media/?root=r&id=i&user=u&version=${version}`
      },
      'u',
      () => current
    );
    ...
    await assert.rejects(source.read(0, 10, new AbortController().signal), /Account changed/);
    assert.equal(canceled, 1);
  } finally {
...
test('stale cloud response rejection does not wait for a non-settling body cancel', async () => {
  ...
  try {
    ...
    await Promise.race([
      assert.rejects(source.read(0, 10, new AbortController().signal), /Account changed/),
      new Promise((_, reject) => setTimeout(() => reject(Error('cloud cleanup stalled')), 200))
    ]);
    assert.equal(canceled, 1);
  } finally {
    globalThis.fetch = fetch;
...
test('runtime mode mismatch is rejected before claiming threaded inference', () => {
  const single = runtime({ HEAP32: new Int32Array(4) }),
    threaded = runtime({ HEAP32: new Int32Array(new SharedArrayBuffer(16)) });
  assert.doesNotThrow(() => assertRuntimeIdentity(single, false));
  assert.doesNotThrow(() => assertRuntimeIdentity(threaded, true));
  assert.throws(() => assertRuntimeIdentity(single, true), /incompatible/);
  assert.throws(() => assertRuntimeIdentity(threaded, false), /incompatible/);
...
test('sub-millisecond container cues export with a positive parseable interval', () => {
  const cues = [
    { id: 'tiny', start: 0.0001, end: 0.0002, text: '短い字幕' },
    { id: 'other', start: 1.1234, end: 1.1235, text: '次の字幕' }
  ];
  for (const format of ['srt', 'vtt']) {
    const out = parseSubtitles(serializeSubtitles({ cues }, format));
    assert.equal(out.length, 2);
    assert.ok(out.every((c) => c.end > c.start));
    assert.equal(out[0].end, 0.001);
  }
});
...
function bindings(changes = {}) {
  ...
  return {
    ...
    _moss_transcribe_capi_last_error() {},
...
test('cancellation pointer must be a non-null aligned in-heap integer', () => {
  for (const offset of [0, -4, 2, 128, Infinity, NaN])
    assert.throws(
      () => assertRuntimeBindings(bindings({ _moss_web_cancel_ptr: () => offset }), false),
      /bindings/
    );
  assert.throws(
    () =>
...
test('worker re-reads and bounds-checks the WASM heap after allocation', () => {
  ...
  assert.ok(
    worker.indexOf('const heap = runtime.HEAPF32') >
      worker.indexOf('runtime._malloc(pcm.byteLength)')
  );
});
test('MOSS operation IDs stay inside the signed 32-bit cancellation contract', () => {
  const client = readFileSync(
    new URL('../../apps/web/src/lib/media/moss-client.ts', import.meta.url),
    'utf8'
  );
...
test('CPU backend graph computation observes the shared cancellation word', () => {
  ...
  const hooks = readFileSync(
    new URL('../../tools/media/manabi_web_hooks.hpp', import.meta.url),
    'utf8'
  );
  const bridge = readFileSync(new URL('../../tools/media/bridge.cpp', import.meta.url), 'utf8');
...
test('threaded runtime must expose deterministic pool cleanup', () => {
  ...
  assert.doesNotThrow(() =>
    assertRuntimeBindings(bindings({ PThread: { terminateAllThreads() {} } }), true)
  );
});
test('full runtime bindings are checked before any weights are requested', () => {
  ...
  assert.ok(
    worker.indexOf('assertRuntimeBindings(runtime!, data.threaded)') <
      worker.indexOf('await getModel(')
  );
});
