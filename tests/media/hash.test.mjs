...
for (const length of [
  ...
  test('SHA-256 matches native across subarray offsets and chunk boundaries: ' + length, () => {
    const storage = new Uint8Array(length + 29),
      bytes = storage.subarray(13, 13 + length);
    for (let i = 0; i < bytes.length; i++) bytes[i] = (i * 31 + (i >>> 8)) & 255;
    assert.equal(new Sha256().update(bytes).hex(), native(bytes));
    const h = new Sha256();
    let step = 1;
    for (let i = 0; i < bytes.length; ) {
      step = (step * 48271) % 65521;
      const end = Math.min(bytes.length, i + (step % 257) + 1);
      h.update(bytes.subarray(i, end));
      i = end;
    }
    assert.equal(h.hex(), native(bytes));
    assert.throws(() => h.update(bytes), /finalized/);
    assert.throws(() => h.hex(), /finalized/);
  });
}
test('blob hash preserves byte identity and supports cancellation between bounded reads', async () => {
  const bytes = new Uint8Array(2 * 1024 * 1024 + 9);
  bytes.fill(157);
  const progress = [];
  assert.equal(
    await hashBlob(new Blob([bytes]), new AbortController().signal, (n) => progress.push(n)),
    native(bytes)
  );
