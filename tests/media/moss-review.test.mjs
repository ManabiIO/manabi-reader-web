...
async function cacheHarness(body) {
  ...
  globalThis.fetch = async () => {
    calls.push('fetch');
    throw Error('Unexpected model network request');
  };
  try {
    ...
    for (const [name, previous] of [
      ...
      if (previous) Object.defineProperty(navigator, name, previous);
      else delete navigator[name];
    }
  }
}
test('Cancel during OPFS directory lookup stops preparation and prevents a late download', () =>
...
test('Cancel during quota estimation stops before opening a writable stream', () =>
  cacheHarness(async ({ mock, calls }) => {
...
test('a writable stream opened after Cancel is aborted without fetching or committing', () =>
  cacheHarness(async ({ file, writer, calls }) => {
...
test('failed fetch closes preparation without waiting indefinitely for writer abort', () =>
  cacheHarness(async ({ writer, calls }) => {
    writer.abort = () => {
      calls.push('abort');
      return new Promise(() => {});
    };
    await assert.rejects(
      bounded(getModel(new AbortController().signal, () => {})),
      /Unexpected model network request/
    );
...
test('2,000 stalled device checkpoints retain just the active and latest write', async () => {
  ...
  await reached.promise;
  const pending = [];
  for (let i = 1; i < 2000; i++) pending.push(device.save(snapshot(i)));
  const closing = device.close(),
    secondClose = device.close();
  let ended = false;
  secondClose.then(() => {
    ended = true;
  });
  try {
    ...
    assert.deepEqual(
      writes.map((value) => value.position),
      [0, 1999]
    );
        ...
        queueMicrotask(() => {
          this.dispatchEvent(
            new MessageEvent('message', {
              data: {
                id: message.id,
                type: message.type === 'prepare' ? 'ready' : 'disposed',
                value: null
              }
            })
          );
...
test('compiled Worker acknowledges only after model and pthread cleanup (runtime doubles)', () => {
  const result = spawnSync(
    process.execPath,
    ['--experimental-vm-modules', 'tests/media/worker-disposal-gate.mjs'],
    {
