...
class WorkerDouble extends EventTarget {
 static current;
 calls=[];
 constructor(){super();WorkerDouble.current=this;}
...
async function withClient(body){
 ...
 try {await body(client);} finally {client.dispose();if(old===undefined)delete globalThis.Worker;else globalThis.Worker=old;}
}
test('early runtime handshake does not settle preparation or admit transcription',()=>withClient(async client=>{
 let prepared=false;
...
test('aborting preparation stores its operation into the early shared cancellation word',()=>withClient(async client=>{
 const controller=new AbortController();
 const pending=client.prepare(controller.signal,()=>{});
 const worker=WorkerDouble.current, prepare=worker.calls[0];
 const buffer=new SharedArrayBuffer(64),word=new Int32Array(buffer,16,1);
...
test('readiness refreshes the shared view and inference cancellation uses its own identity',()=>withClient(async client=>{
 const pending=client.prepare(signal(),()=>{}),worker=WorkerDouble.current;
 const initial=new SharedArrayBuffer(64),current=new SharedArrayBuffer(128);
 worker.reply('runtime',{buffer:initial,offset:16});
 worker.reply('ready',{buffer:current,offset:16});
 await pending;
 const controller=new AbortController(),transcribing=client.transcribe(new Float32Array(10),controller.signal);
...
])test(`invalid early cancellation ${name} rejects before readiness`,()=>withClient(async client=>{
 const pending=client.prepare(signal(),()=>{}),rejected=assert.rejects(pending,/cancellation buffer/);
 WorkerDouble.current.reply('runtime',value);
 await rejected;
 await assert.rejects(client.transcribe(new Float32Array(10),signal()),/Prepare/);
}));
test('retired preparation messages cannot make its replacement model ready',()=>withClient(async client=>{
 const controller=new AbortController(),first=client.prepare(controller.signal,()=>{});
 ...
 await rejected;
 const second=client.prepare(signal(),()=>{}),current=WorkerDouble.current;
...
test('worker exposes preparation cancellation before fetching or mounting weights (source integration guard)',()=>{
 const text=fs.readFileSync(new URL('../../apps/web/src/lib/media/moss-worker.ts',import.meta.url),'utf8');
 const begin=text.indexOf('runtime!._moss_web_begin(operation)'),handshake=text.indexOf("reply('runtime'");
