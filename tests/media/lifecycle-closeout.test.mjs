...
async function harness(body) {
  ...
  const queues = [];
  const make = (engine) => {
    const q = new TranscriptionQueue(store, 'guest', engine, async () => new Float32Array(16000).fill(.1));
    queues.push(q);
    return q;
  };
  try { await body({ store, make }); }
  finally {
...
function contains(error, expected) {
  return error === expected || error instanceof AggregateError && error.errors.some(e => contains(e, expected));
}

test('Close reports the draining batch retirement failure and retires the engine once', () => harness(async ({ make }) => {
  const started = deferred(), failure = Error('runtime retirement failed');
  let stops = 0;
  const q = make(preparing(started, () => { stops++; throw failure; }));
...
test('Close reports a failed pause transaction even though the active signal is aborted', () => harness(async ({ make, store }) => {
  const started = deferred(), failure = Error('pause checkpoint disk failure');
  const original = store.updateLocal.bind(store);
  store.updateLocal = (scope, kind, id, change) => original(scope, kind, id, old => {
...
test('Close drains an admitted heartbeat operation and fences its delayed renewal', () => harness(async ({ make, store }) => {
  const started = deferred(), renewal = deferred();
  const set = globalThis.setInterval, clear = globalThis.clearInterval;
  let heartbeat, ticking = false, cleared = false, done = false;
  globalThis.setInterval = (fn, ms) => { assert.equal(ms, 15000); heartbeat = fn; return 1; };
  globalThis.clearInterval = () => { cleared = true; };
  const original = store.updateLocal.bind(store);
  store.updateLocal = (...args) => ticking ? renewal.promise.then(() => original(...args)) : original(...args);
  const q = make(preparing(started));
  let closing;
  try {
    const job = await q.enqueue(key, 'en', '1', 2);
    await started.promise;
    ticking = true; heartbeat(); ticking = false;
    closing = q.dispose().then(() => { done = true; });
...
test('Close waits for initial recovery and prevents a delayed scan from mutating jobs', () => harness(async ({ make, store }) => {
  const scan = deferred(), entered = deferred();
  const original = store.listLocal.bind(store);
  let updates = 0, done = false;
  const update = store.updateLocal.bind(store);
  store.updateLocal = (...args) => { updates++; return update(...args); };
  store.listLocal = async (...args) => { entered.resolve(); await scan.promise; return original(...args); };
