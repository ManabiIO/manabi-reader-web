...
test('a title match cannot bind an unrelated browser book to an external source', () => {
  ...
  assert.doesNotThrow(() => assertExternalBookSource({ storageSource: 'selected-library' }, 'selected-library'));
});
...
test('logical TTU revisions are unambiguous, version-qualified and left intact', () => {
  ...
  for (const name of ['bookdata_2_6_1_1_1.zip', 'bookdata_1_7_1_1_1.zip', 'bookdata_1_6_1_1_1.json']) {
    assert.throws(() => selectTtuFile([{ name }], 'bookdata_'), /Unsupported TTU file format/);
  }
  assert.throws(() => selectTtuFile([{ name: 'bookdata_1_6_9007199254740992_1_1.zip' }], 'bookdata_'), /Invalid numeric/);
...
test('native statistics, exponential progress and textual anchors keep the TTU wire contract', () => {
  const names = [
    ...
    ['statistics_', 'statistics_1_6_1789837322544_60_300_720_720_720_720_300_300_60_60_720_720_na.json']
  ];
