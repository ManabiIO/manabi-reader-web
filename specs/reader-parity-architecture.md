...
### Next data migration: statistics identity

The inherited `statistic` store has a `[title, dateKey]` primary key. A schema migration must add content-keyed day rows without guessing which of two same-titled books owns an old row. Migrate an old title only when its known copies resolve to exactly one logical content hash; preserve ambiguous and unhashed old rows as recoverable legacy data. New tracking for a verified book must write its `content:<sha256>` row from the first day onward, even if another book has the same title. The tracker, global statistics view, reading goals, native export/import adapter, completion statistics, and personal sync must all consume the same content-keyed data or explicitly show legacy-orphan rows, avoiding double counts. The migration and repeat runs must be idempotent; a title rename or provider move must never change the statistic key. Browser acceptance needs two different books with the same title and different read sessions, a move/relink, account sync, and an export/import round trip. Do not relax the current fail-closed behavior merely to make a status green before that migration is finished.
