...
Still open before calling features 1–5 complete: composed account-switch and offline sync browser coverage, migration of title-keyed legacy statistics to content identity, composed OneDrive series interruption and receipt replay, and the broader locator/search/Line Guide matrix above. Until statistics migrate, personal sync fails closed when distinct books share a legacy statistics title; it never attributes that title's rows to either content hash. Google Drive and Dropbox physical mutations remain unavailable under current provider capabilities.
