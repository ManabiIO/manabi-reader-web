...
Focused IndexedDB tests cover idempotence, same-title ambiguity, conflicting renamed titles and local-to-content promotion. Browser tests cover separate same-title account mutations, retention of ambiguous legacy days, TTU round trips and interruption/retry behavior. This branch remains draft until the full CI and browser matrix pass. A remaining product decision is how to expose unresolved legacy days in the statistics UI without conflating them with verified rows or silently losing them from portable exports. Device-level provider move/relink and deletion preference coverage remain open.
