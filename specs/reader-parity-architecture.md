...
The highest-priority browser gate is an offscreen search result in another EPUB spine resource followed by reflow and exact Return. A built-static Chromium/WebKit regression now covers that path with byte-identical bookmark/statistics evidence; the broader locator and Line Guide matrix remains open. Cloud series has a composed frontend/backend interruption test against PostgreSQL, and its current-head run must pass before release.
...
Still open before calling features 1–5 complete: the broader locator/search/Line Guide matrix above, device-level Safari offline reload, and clean current-head composed qualification. The content-keyed statistics migration is implemented in the follow-up branch described below. Ambiguous inherited title-keyed days remain unassigned and are never attributed to either content hash for account sync. Google Drive and Dropbox physical mutations remain unavailable under current provider capabilities.

Direct browser imports now preserve two different original-file hashes even when both EPUBs have the same title. The Library lists both IDs, and removing one by ID leaves the other and inherited title-keyed auxiliary data intact. New statistics use logical book keys; ambiguous inherited title-keyed days retain their explicit recovery path rather than being assigned to either book.
...
The backend series suite now drives the public HTTP prepare/execute/status endpoints against a stateful OneDrive fake. It loses a move response after the provider has moved the original, verifies the journal reports reconciliation without an early receipt, resumes after the claim ages, proves the move is not repeated, and checks that only the later verification publishes the receipt. Another account cannot read the plan. A separate built-static frontend browser regression starts with an old cloud catalog, exposes the backend's completed plan after an interrupted browser session, and verifies that two relocation receipts are applied once, the catalog is refreshed, no execute request is repeated, and unrelated IndexedDB reading data is unchanged. It passes in local Chromium and WebKit and in frontend Linux CI. The backend's composed browser/PostgreSQL test also creates a series through the real UI, loses a OneDrive move response, reloads the page, and reconciles without moving the same file twice. That test passed on an earlier head; require it to pass on the release head.
...
Focused IndexedDB tests cover idempotence, same-title ambiguity, conflicting renamed titles, local-to-content promotion, and raw recovery serialization. Browser tests cover separate same-title account mutations, retention of ambiguous legacy days, TTU round trips, interruption/retry behavior, and the raw download versus ambiguous ZIP gate. The follow-up branch's 12 exact-head frontend checks pass, but it remains draft pending the composed backend acceptance and broader reader matrix. A remaining product decision is how to expose unresolved legacy days in the statistics UI without conflating them with verified rows. Device-level provider move/relink and deletion preference coverage remain open.
