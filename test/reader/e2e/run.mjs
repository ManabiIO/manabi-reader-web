...
async function importBook(filename, title) {
    ...
    if (title) { await expect(page.getByText(title, {exact: true}).first()).toBeVisible({timeout: 60000}); await expect(page.locator('[title="Import Files"]').first()).toBeEnabled(); }
}
...
async function controlSW() {
    await page.waitForFunction(async () => (await navigator.serviceWorker.getRegistration())?.active?.state === 'activated', undefined, {timeout: 90000});
...
try {
    ...
    if (shell) {
        ...
        if (imported) {
            ...
            if (opened) {
                ...
                await check('reader: bookmark is written through the reading UI', async () => { await page.locator('.book-content').first().click({position: {x: 30, y: 30}}); for (let i=0;i<3;++i) { const before=await readingPosition(); await page.keyboard.press('PageDown'); await expect.poll(readingPosition).toBeGreaterThan(before); } readingOffset=await readingPosition(); await page.keyboard.press('b'); await expect.poll(async () => (await getBookmarks()).length).toBeGreaterThan(0); await expect.poll(async () => Math.max(...(await getBookmarks()).map((b) => b.exploredCharCount))).toBeGreaterThan(0); bookmarkSnapshot = await getBookmarks(); return {bookmarks: bookmarkSnapshot, readingOffset}; });
                await check('reader: book and bookmark survive reload', async () => { await page.reload(); await expect(page.locator('.book-content')).toContainText('猫'); assert.deepEqual(await getBookmarks(), bookmarkSnapshot); await expect.poll(readingPosition).toBeGreaterThan(0); await expect.poll(async () => Math.abs((await readingPosition())-readingOffset)).toBeLessThanOrEqual(2); });
            ...
            await check('reader: full browser restart retains books and bookmark', async () => { await page.goto(bookURL || origin + '/manage'); await expect(page.locator('.book-content')).toContainText('猫'); assert.deepEqual(await getBookmarks(), bookmarkSnapshot); await expect.poll(readingPosition).toBeGreaterThan(0); await expect.poll(async () => Math.abs((await readingPosition())-readingOffset)).toBeLessThanOrEqual(2); });
        ...
        await check('reader: HTMLZ import uses production archive workers and renderer', async () => { await importBook('htmlbook.htmlz', 'E2E HTMLZ'); await openBook('E2E HTMLZ'); await expect(page.locator('.book-content rt').first()).toHaveText('がっこう'); await expect(page.locator('.book-content h1')).toHaveText('E2E HTMLZ'); await expect(page.locator('.book-content')).toContainText('導入の文章'); await expect(page.locator('.book-content')).toContainText('末尾の文章'); });
        await check('reader: UTF-8 text import and rendering', async () => { await importBook('plain.txt', 'plain'); await openBook('plain'); await expect(page.locator('.book-content')).toContainText('学校'); });
        ...
        await check('security: custom font metadata cannot inject a remote font URL', async () => {
            ...
            try { await page.reload(); await expect(page.getByText('E2E Hostile EPUB', {exact: true}).first()).toBeVisible(); assert.equal(await page.locator('style#ttu-userfonts').count(), 0); }
            finally { await page.evaluate(() => localStorage.removeItem('userfonts')); await page.reload(); }
        });
        await check('security: selected font family cannot inject a second CSS declaration', async () => {
    ...
    const ready = await check('manabitan: fresh real extension starts without preinstalled dictionaries', async () => { let sw = context.serviceWorkers().find((w) => w.url().startsWith('chrome-extension:')); if (!sw) { sw = await context.waitForEvent('serviceworker', {predicate: (w) => w.url().startsWith('chrome-extension:')}); } extensionURL = new URL(sw.url()).protocol + '//' + new URL(sw.url()).host; await openSettings(); assert.deepEqual(await send('getDictionaryInfo'), []); });
    if (ready) {
        const installed = await check('manabitan: install full standard JMdict English through settings UI', async () => { const metadata = JSON.parse(await fs.readFile(path.join(fixtures, 'jmdict.json'))); await settings.evaluate(() => { globalThis.__manabitanImportCompletionSignalEnabled = true; }); await settings.setInputFiles('#dictionary-import-file-input', path.join(fixtures, 'JMdict_english.zip')); const deadline = Date.now() + 600000; let completion; while (Date.now() < deadline) { const state = await settings.evaluate(() => ({completion: globalThis.__manabitanLastImportCompletion, error: document.querySelector('#dictionary-error:not([hidden])')?.textContent?.trim(), busy: !!document.querySelector('#dictionaries-modal .dictionary-import-progress:not([hidden]), #recommended-dictionaries-modal .dictionary-import-progress:not([hidden])')})); if (state.error) { throw new Error(state.error); } if (state.completion && !state.busy) { completion = state.completion; break; } await settings.waitForTimeout(300); } assert.ok(completion, 'Import must finish, not merely create a dictionary row'); assert.equal(completion.errorCount || 0, 0); const info = await send('getDictionaryInfo'); installedTitle = info.find((d) => /JMdict/i.test(d.title))?.title; assert.ok(installedTitle); const counts = await send('getDictionaryCounts', {dictionaryNames: [installedTitle], getTotal: false}); assert.equal(counts.counts[0].terms, metadata.termRows); return {completion, info, counts, fixture: metadata}; });
        if (installed) {
            ...
            if (imported) {
                ...
                await check('integration: repeated scans leave one visible popup', async () => { for (const term of ['学校', '猫', '学校']) { await hoverWord(term); } assert.equal((await visiblePopupFrames()).length, 1); });
                await check('integration: a second Reader tab shares extension dictionary safely', async () => {
    ...
    await check('fonts: no external Google Fonts stylesheet or binary requests', async () => { assert.deepEqual(remoteFontRequests, []); });
    await check('browser: no uncaught application page exceptions', async () => { assert.deepEqual(browserErrors, []); });
