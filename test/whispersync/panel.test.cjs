...
const source =
  process.env.WHISPERSYNC_COMPONENT_SOURCE ||
  join(__dirname, '../../apps/web/src/lib/features/whispersync');
const deferred = () => {
  let resolve, reject;
...
async function harness(options = {}) {
  ...
  const fakeIndex = {
    ...
    isCurrent: () => true,
    dispose() {
      this.disposals = (this.disposals || 0) + 1;
    }
  };
  const matched = options.matched || { promise: Promise.resolve([{ start: 0, end: 8, score: 1 }]) };
  class Store {
    load() {
      return options.load || Promise.resolve(options.session || savedSession());
    }
    save(key, session) {
      const pending = deferred();
      saves.push({ key, session, ...pending });
  ...
  class Player {
    ...
    constructor(environment, changed) {
  ...
  let script = readFileSync(join(source, 'audiobook-panel.svelte'), 'utf8').match(
    /<script lang="ts">([^]*?)<\/script>/
  )[1];
  script = script
    .replace(/^ {2}export let bookId: number$/m, '  let bookId: number = 1')
    .replace(/^ {2}export let bookTitle: string$/m, "  let bookTitle: string = 'Book'")
    .replace(/^ {2}export let /gm, '  let ')
    .replace(/^ {2}\$:.*$/gm, '');
...
test('subtitle parse failure preserves independently valid audio resume metadata', async () => {
  const h = await harness({
    session: savedSession({ subtitleSource: 'invalid subtitles', subtitleName: 'bad.srt' })
  });
  assert.equal(h.inspect().saved.position, 10);
