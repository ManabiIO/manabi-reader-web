...
try {
  const program = ts.createProgram(files, {
    ...
    types: [], outDir: output, skipLibCheck: false
  })
  ...
  if (diagnostics.length) {
    ...
    const result = spawnSync(process.execPath, ['--test', ...readdirSync(join(root, 'test/whispersync')).filter((file) => file.endsWith('.test.cjs')).map((file) => join(root, 'test/whispersync', file))], {
      stdio: 'inherit', env: { ...process.env, WHISPERSYNC_COMPILED: output }
    })
    status = result.status ?? 1
    if (process.argv.includes('--svelte')) {
      const webRequire = createRequire(join(root, 'apps/web/package.json'))
      const compiler = webRequire('svelte/compiler')
      const components = readdirSync(source).filter((file) => file.endsWith('.svelte')).map((file) => join(source, file))
      components.push(join(root, 'apps/web/src/routes/b/+page.svelte'))
      for (const file of components) for (const generate of ['client', 'server']) {
        const result = compiler.compile(readFileSync(file, 'utf8'), { filename: file, generate })
        console.log(`${file}: ${generate} compiled (${result.warnings.length} warnings)`)
    ...
    if (bundleArgument) {
      ...
      const modules = readdirSync(output).filter((file) => file.endsWith('.js')).map((file) => {
