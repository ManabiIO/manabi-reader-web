...
window.runWhispersyncBrowserTests = async function ({ skipStorage = false } = {}) {
  ...
  await test('noninteger and nonfinite range offsets fail safely', async () => {
    ...
    for (const start of [NaN, Infinity, 0.5])
      equal(w.rangeForMatch(index, { start, end: 5, score: 1 }), undefined);
  });
  await test('modal aria-hidden outside the book does not exclude its narration', async () => {
    ...
    wrapper.append(root);
    try {
      equal((await w.buildBookIndex(root)).text, 'originalpassage');
    } finally {
  ...
  await test('a mutation during cue matching invalidates the complete candidate index', async () => {
    ...
    const index = await w.buildBookIndex(root);
    const promise = w.matchCues(
      index.text,
      Array.from({ length: 40 }, () => makeCue('abcdefghij'))
    );
  ...
  if (!skipStorage)
    await test('IndexedDB roundtrip persists only declared session fields', async () => {
  ...
  if (!skipStorage)
    await test('queued saves capture immutable snapshots and deletion runs last', async () => {
      const store = new w.AudiobookSessionStore();
  ...
  if (!skipStorage)
    await test('storage shutdown waits for pending saves', async () => {
      const store = new w.AudiobookSessionStore();
