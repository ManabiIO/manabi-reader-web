...
const { AudiobookSessionStore, emptySession } = require(
  join(process.env.WHISPERSYNC_COMPILED, 'persistence.js')
);
const drain = async () => {
  ...
  const db = {
    ...
    transaction(name, mode) {
      ...
      const tx = {
        ...
        fail() {
          this.error = new Error('quota failure');
          this.abort();
        }
      };
...
test('load waits for an earlier write and saves an immutable caller snapshot', async () => {
  const db = database(),
    store = new AudiobookSessionStore(factory(db));
  const value = {
    ...emptySession(),
    position: 12,
    audio: { name: 'one.mp3', size: 10, lastModified: 1 }
  };
...
test('remove follows pending writes and a queued read observes the deletion', async () => {
  ...
  const saving = store.save('book', emptySession()),
    removing = store.remove('book'),
    loading = store.load('book');
  await drain();
  db.transactions[0].complete();
  await saving;
  await drain();
...
test('blocked opening rejects, aborts a late upgrade, closes a late success and permits retry', async () => {
  ...
  const loading = store.load('book'),
    rejected = assert.rejects(loading, /blocked/);
  await drain();
...
test('a version change discards the old cached handle before subsequent operations', async () => {
  const db = database(),
    source = factory(db),
    store = new AudiobookSessionStore(source);
