...
const { parseTimestamp, parseSubtitles, cueText, CueTimeline, MAX_SUBTITLE_BYTES } = require(join(compiled, 'subtitles.js'))
...
const { emptySession, validateSession, sessionKey, AudiobookSessionStore } = require(join(compiled, 'persistence.js'))
...
test('malformed and out-of-range timestamps are rejected', () => {
  for (const invalid of ['-00:00:01.000', '00:60:00.000', '01:02:60.000', '00:01.1', 'NaN', 'Infinity', '1:2:3', '00:01.000junk']) {
...
test('BOM, CRLF, out-of-order cues and duplicate external IDs', () => {
  ...
  assert.deepEqual(result.map((item) => [item.text, item.start, item.id]), [['First', 1, 1], ['Second', 4, 0]])
})
test('VTT metadata, comments, cue identifiers and cue settings', () => {
  const result = parseSubtitles('WEBVTT example\nKind: captions\n\nNOTE note\nignored\n\nSTYLE\n::cue {color: red}\n\nREGION\nid:foo\n\nintro\n00:01.000 --> 00:03.000 position:20%\n<v John><b>Hello</b> &amp; goodbye</v>')
...
test('empty, incomplete, reversed, zero-duration and partly malformed files fail atomically', () => {
  for (const invalid of ['', 'WEBVTT\n', '1\nNo time', '00:00:02.000 --> 00:00:01.000\nBad', '00:00:01.000 --> 00:00:01.000\nBad', '00:00:01.000 --> 00:00:02.000\n', '00:00:01.000 --> 00:00:02.000\nGood\n\ninvalid']) assert.throws(() => parseSubtitles(invalid))
})
...
test('cue timeline handles boundaries, real gaps and nested overlaps', () => {
  const timeline = new CueTimeline([cue('outer', 0, 20, 0), cue('inner', 2, 3, 1), cue('later', 22, 25, 2)])
...
test('exact matching is forward-only across multiple cues', async () => {
  const result = await matchCues(normalizeText('前書き。これは最初。これは次です。'), [cue('これは最初'), cue('これは次です')])
  assert.deepEqual(result, [{ start: 3, end: 8, score: 1 }, { start: 8, end: 14, score: 1 }])
})
test('unmatched cues do not move the cursor past a later match', async () => {
  const result = await matchCues('beginningafterwards', [cue('not present'), cue('beginning'), cue('afterwards')])
...
test('short ambiguous captions cannot anchor to random distant text', async () => {
  ...
  assert.deepEqual((await matchCues('はいx', [cue('はい')], { start: 0 }))[0], { start: 0, end: 2, score: 1 })
})
test('bounded search requires a start hint beyond the forward-search limit', async () => {
  ...
  assert.equal((await matchCues(text, [cue('startofchapter')], { start: 25_000 }))[0].start, 25_000)
})
...
test('aborted work does not publish final progress or return results', async () => {
  ...
  controller.abort()
  let progress = false
  await assert.rejects(matchCues('hello', [cue('hello')], { signal: controller.signal, onProgress: () => { progress = true } }), { name: 'AbortError' })
  assert.equal(progress, false)
})
test('matching yields and supports cancellation partway through a long cue list', async () => {
  const controller = new AbortController()
  const promise = matchCues('abcdef'.repeat(500), Array.from({ length: 500 }, () => cue('abcdef')), { signal: controller.signal, onProgress: () => controller.abort() })
  await assert.rejects(promise, { name: 'AbortError' })
})
test('session validation rejects corrupt, non-finite and oversized persisted values', () => {
  for (const value of [{}, { ...emptySession(), rate: NaN }, { ...emptySession(), delay: Infinity }, { ...emptySession(), rate: 20 }, { ...emptySession(), audio: { name: 'a', size: -1, lastModified: 0 } }]) assert.throws(() => validateSession(value))
})
test('session validation copies nested data and does not retain extra fields', () => {
  const original = { ...emptySession(), audio: { name: 'a', size: 100, lastModified: 1 }, bogus: 'not persisted' }
...
class FakeAudio extends EventTarget {
  constructor() { super(); this.currentTime = 0; this.duration = 120; this.paused = true; this.seeking = false; this.playbackRate = 1; this.style = {}; this.attributes = {}; this.src = ''; this.loads = 0 }
  ...
  play() { if (this.rejectPlay) return Promise.reject(new Error('NotAllowedError')); this.paused = false; this.fire('play'); return this.playPromise ?? Promise.resolve() }
}
function harness() {
  const audios = [], revoked = [], frames = new Map(), states = [], attached = []
  let serial = 0
  const player = new LocalAudioPlayer({
    ...
    revokeURL: (url) => revoked.push(url), attach: (audio) => attached.push(audio),
    requestFrame: (fn) => { const id = serial++; frames.set(id, fn); return id },
...
test('stale play rejection cannot overwrite the new file state', async () => {
  ...
  let reject
  h.player.load(h.file())
  h.audios[0].playPromise = new Promise((_, r) => { reject = r })
...
test('invalid and fully out-of-duration loops are not installed', () => {
  ...
  h.player.load(h.file())
  h.audios[0].fire('loadedmetadata')
  for (const bounds of [[3, 2], [-10, -1], [121, 123], [NaN, 1]]) {
...
test('opening the panel or loading metadata cannot overwrite the existing resume point', () => {
  ...
  const previous = { ...emptySession(), audio: { name: 'saved.mp3', size: 500, lastModified: 1 }, position: 45 }
...
test('closing the media preserves the captured latest checkpoint rather than a stale save', () => {
  const { captureSession } = require(join(compiled, 'persistence.js'))
  const previous = { ...emptySession(), audio: { name: 'saved.mp3', size: 500, lastModified: 1 }, position: 45 }
  const playing = { file: previous.audio, time: 48.5, duration: 300, ready: true, paused: false, rate: 1.25 }
  const checkpoint = captureSession(previous, playing, previous)
  const closed = captureSession(previous, { ...playing, file: undefined, ready: false, time: 0 }, checkpoint)
...
test('late rejection from an earlier play attempt cannot poison a successful retry', async () => {
  ...
  h.player.load(h.file())
  let reject
  h.audios[0].playPromise = new Promise((_, fail) => { reject = fail })
...
test('denied IndexedDB getter is reported asynchronously without breaking player construction', async () => {
  ...
  Object.defineProperty(globalThis, 'indexedDB', { configurable: true, get() { throw new Error('access denied') } })
  try {
    let store
    assert.doesNotThrow(() => { store = new AudiobookSessionStore() })
