...
import {
  installationPlan,
  processingSetFingerprint,
  DictionaryInstallationCoordinator
} from '../../apps/web/src/lib/dictionary-setup/installation.mjs';
...
function session(provider = null, extra = {}) {
...
test('identity path returns original bytes, no mode, cache access, runtime mount, or background work', async () => {
  ...
  const s = session(
    ...
    { cache: { get: () => cacheCalls++, put: () => cacheCalls++ } }
  );
  const c = new ChapterOpeningController({
    session: s,
    stage: async (result) => {
      assert.equal(result.html, chapter.html);
      return { root: {}, commit: () => committed++, rollback: () => {} };
    }
  });
  const r = await c.open(0);
...
test('intentional null result remains unchanged and is not cached as an annotated success', async () => {
  ...
  const s = session(
    ...
    { cache: { get: async () => null, put: () => writes++ } }
  );
  for (let n = 0; n < 2; n++) {
    const r = await s.open(0);
    assert.equal(r.result.html, chapter.html);
...
test('undefined and failures do not become no-op success', async () => {
  for (const preprocess of [
    ...
    const s = session(active({ preprocess }));
    await assert.rejects(s.open(0));
    s.dispose();
  }
  assert.throws(() =>
    normalizeChapterResult(
      { protocol: 1, fingerprint: 'f', html: 'changed', sidecar: null, vocabulary: [] },
      chapter.html,
      'f'
    )
  );
...
test('unavailable generation cannot return annotations', async () => {
  const s = session(active({ fingerprint: async () => null, preprocess: async (i) => output(i) }));
  await assert.rejects(s.open(0), /Annotations require/);
  s.dispose();
});
test('resource installation while staging unchanged chapter prevents stale reveal', async () => {
  let f = null,
    committed = 0;
  const s = session(active({ fingerprint: async () => f }));
  const c = new ChapterOpeningController({
    session: s,
    stage: async () => {
      f = 'installed';
...
test('same processor becomes active at next opening without a reader-mode toggle', async () => {
  let f = null;
  const s = session(
    active({ fingerprint: async () => f, preprocess: async (i) => (f ? output(i) : null) })
  );
  assert.equal((await s.open(0)).result.sidecar, null);
  f = 'ready';
  assert.deepEqual((await s.open(0)).result.sidecar, { fixture: true });
  s.dispose();
});
...
const asset = (id, purpose = 'definitions', extra = {}) => ({
  id,
  label: id,
  purpose,
  revision: 'r1',
  contentSha256: 'a'.repeat(64),
  source: 'https://manabi.io/' + id,
  ...extra
});
const sk = asset('skeleton', 'processing'),
  def = asset('jitendex');
const receipt = (a) => ({
  ...a,
  artifactID: 'artifact-' + a.id,
...
function storage(overrides = {}) {
  ...
  const db = {
    ...
    withLock: async (fn) => {
      ...
      calls.push('ensure:' + a.id);
      return receipt(a);
    },
    activateProcessingSet: async (s) => {
      calls.push('activate');
      set = s;
    },
    readProcessingSet: async () => {
      calls.push('readback');
      return set;
    },
    ...overrides
...
test('not-now/empty selection makes no install or storage request', async () => {
  const db = storage();
  const c = new DictionaryInstallationCoordinator({ processingAssets: [sk], storage: db });
  assert.deepEqual(await c.install([]), { installed: [], processingFingerprint: null });
  assert.deepEqual(db.calls, []);
});
test('every user-selected dictionary implies required processing resources first', () => {
  for (const id of ['jitendex', 'custom-local', 'frequency-only', 'jp-jp'])
    assert.deepEqual(
      installationPlan([asset(id)], [sk]).map((a) => a.id),
      ['skeleton', id]
    );
});
...
test('identical requests are deduplicated without mutating caller input', () => {
  const plan = installationPlan([def, def], [sk, sk]);
  def.label = 'renamed';
  assert.equal(plan.length, 2);
  assert.notEqual(plan[1].label, def.label);
  def.label = 'jitendex';
});
test('processing set is installed, activated and verified before definitions', async () => {
  const db = storage();
  const c = new DictionaryInstallationCoordinator({ processingAssets: [sk], storage: db });
  const result = await c.install([def]);
  assert.deepEqual(db.calls, [
    'lock',
    'ensure:skeleton',
    'activate',
  ...
  assert.deepEqual(result.installed, ['skeleton', 'jitendex']);
  assert.match(result.processingFingerprint, /^processing-set-v2:/);
});
test('public distribution needs neither private engine nor processing catalogue', async () => {
  const db = storage();
  await new DictionaryInstallationCoordinator({ storage: db }).install([def]);
...
test('failed skeleton import prevents new activation and definition work', async () => {
  ...
  const db = storage({
      ...
      if (a.purpose === 'processing') throw Error('bad');
      defs++;
      return receipt(a);
    },
    activateProcessingSet: async () => activated++
  });
  await assert.rejects(
    new DictionaryInstallationCoordinator({ storage: db, processingAssets: [sk] }).install([def])
  );
...
test('a false receipt never establishes readiness', async () => {
  for (const bad of [
    ...
    assert.ok(!db.calls.includes('activate'));
  }
});
test('processing readback mismatch prevents claiming completion', async () => {
  ...
  assert.ok(!db.calls.includes('ensure:jitendex'));
});
...
test('retry after definition failure can reuse the verified skeleton receipt', async () => {
  ...
  const retained = new Map(),
    db = storage({
        ...
        if (a.id === def.id && fail) {
          fail = false;
          throw Error('definition download');
        }
        if (!retained.has(a.id)) {
          imports++;
          retained.set(a.id, receipt(a));
        }
        return retained.get(a.id);
      }
    });
  const c = new DictionaryInstallationCoordinator({ storage: db, processingAssets: [sk] });
  await assert.rejects(c.install([def]));
  await c.install([def]);
  assert.equal(imports, 2);
});
test('analysis identity ignores display labels/order but binds content and importer semantics', async () => {
  const a = receipt(sk),
    b = receipt(asset('morphology', 'processing'));
  const f = await processingSetFingerprint([a, b]);
  assert.equal(
    f,
    await processingSetFingerprint([
      { ...b, label: 'new' },
      { ...a, artifactID: 'different' }
    ])
  );
  for (const change of [
    { contentSha256: 'b'.repeat(64) },
    ...
    assert.notEqual(f, await processingSetFingerprint([{ ...a, ...change }, b]));
...
const archiveAsset = async () => ({
  ...sk,
  contentSha256: await sha256(zip),
  sourceRepresentation: 'brotli-wrapped-zip'
});
const brotliStream = (stream) => {
  const parts = [];
  return stream.pipeThrough(
    new TransformStream({
      ...
      flush(c) {
        c.enqueue(new Uint8Array(brotliDecompressSync(globalThis.Buffer.concat(parts))));
      }
    })
  );
};
test('already HTTP-decoded Brotli archive is not decompressed twice', async () => {
  let decoded = 0;
  const bytes = await fetchVerifiedDictionaryArchive(await archiveAsset(), {
    fetcher: async () => new Response(zip, { headers: { 'Content-Encoding': 'br' } }),
    brotliStream: () => {
      decoded++;
      throw Error('double decode');
    }
  });
...
test('raw Brotli payload is decompressed and checksum-verified as ZIP bytes', async () => {
  const bytes = await fetchVerifiedDictionaryArchive(await archiveAsset(), {
    fetcher: async () => new Response(brotliCompressSync(zip)),
    brotliStream
  });
  assert.deepEqual(bytes, zip);
});
test('bad checksum and unapproved origin cannot be admitted', async () => {
  const a = await archiveAsset();
  await assert.rejects(
    fetchVerifiedDictionaryArchive(
      { ...a, contentSha256: 'b'.repeat(64) },
...
test('archive transport and decompressed budgets are independently enforced', async () => {
  const a = await archiveAsset();
  await assert.rejects(
    fetchVerifiedDictionaryArchive(a, {
      fetcher: async () => new Response(zip),
      maxTransferBytes: 4
    }),
    /budget/
...
test('unsupported Brotli is an error, not an empty dictionary or fallback import', async () => {
  await assert.rejects(
    fetchVerifiedDictionaryArchive(await archiveAsset(), {
      fetcher: async () => new Response([1, 2, 3]),
      brotliStream: () => {
...
test('aborting a stalled dictionary body cancels the stream', async () => {
  ...
  const pending = fetchVerifiedDictionaryArchive(await archiveAsset(), {
    signal: c.signal,
